...
def assign_slits(slf, binarr, edgearr, lor=-1):
    """
    ...
    while True:
        ...
        while True:
            ...
            for ii in range(pks.size):
                ...
                strev = strev[:-1] + ")"
                widx = eval(strev)
                if widx[0].size < 2*slf._argflag['trace']['slits']['polyorder']:
                    continue
                badmsk, fitcof = arutils.robust_polyfit(widx[0], widx[1],
                                                        slf._argflag['trace']['slits']['polyorder'],
                                                        function=slf._argflag['trace']['slits']['function'],
                ...
                for vv in range(vals.size):
                    ...
                    diff = widx[1] - arutils.func_val(fitcof, widx[0],
                                                      slf._argflag['trace']['slits']['function'],
                                                      minv=0, maxv=binarr.shape[0]-1)
...
#    plybin = arcyutils.bin_x(slf._pixlocn[det-1][:,:,1], binby, 1)
    ...
    medrep = 3
    if len(slf._argflag['trace']['slits']['single']) > 0:
        ...
        if slf._argflag['trace']['slits']['single'][redge] > 0:
            msgs.warn("Using input slit edges on detector {:d}: [{:g},{:g}]".format(
                    det,
                    slf._argflag['trace']['slits']['single'][ledge],
                    slf._argflag['trace']['slits']['single'][redge]))
            msgs.warn("Better know what you are doing!")
            edgearr[:, slf._argflag['trace']['slits']['single'][ledge]] = -1
            edgearr[:, slf._argflag['trace']['slits']['single'][redge]] = +1
    else:
        ...
        if slf._argflag['trace']['slits']['number'] > 0:
            ...
            for ii in range(0, slf._argflag['trace']['slits']['number']):
    ...
    if slf._argflag['trace']['slits']['maxgap'] is not None:
    ...
    if lcnt == 1:
        ...
        assign_slits(slf, binarr, edgearrcp, lor=-1)
    # Assign right edges
    ...
    if slf._argflag['trace']['slits']['maxgap'] is not None:
        ...
        hasedge = arcytrace.close_edges(edgearrcp, vals, int(slf._argflag['trace']['slits']['maxgap']))
        ...
        if edgedup.size > 0:
            for jj in range(edgedup.size):
                ...
                arcytrace.dual_edge(edgearr, edgearrcp, wdup[0], wdup[1], wvla, wvlb, shadj,
                                    int(slf._argflag['trace']['slits']['maxgap']), edgedup[jj])
        # Now introduce new edge locations
        vals = np.sort(np.unique(edgearrcp[np.where(edgearrcp != 0)]))
        edgearrcp = arcytrace.close_slits(binarr, edgearrcp, vals, int(slf._argflag['trace']['slits']['maxgap']))
    # Update edgearr
    ...
    while iterate:
        ...
        #msgs.info("Ignoring any slit that spans < {0:3.2f}x{1:d} pixels on the detector".format(slf._argflag['trace']['slits']['fracignore'], int(edgearr.shape[slf._dispaxis]*binby)))
        msgs.info("Ignoring any slit that spans < {0:3.2f}x{1:d} pixels on the detector".format(slf._argflag['trace']['slits']['fracignore'], int(edgearr.shape[0])))
        fracpix = int(slf._argflag['trace']['slits']['fracignore']*edgearr.shape[0])
        ...
        rmax -= rxc
        iterate = False
        if slf._argflag['trace']['slits']['number'] == 1:  # Another check on slits for singleSlit
            if lmax < lmin:
    ...
    lcoeff = np.zeros((1+slf._argflag['trace']['slits']['polyorder'], lmax-lmin+1))
...
#    minvf, maxvf = slf._pixlocn[det-1][0, 0, 0], slf._pixlocn[det-1][-1, 0, 0]
    for i in range(lmin, lmax+1):
        ...
        if np.size(w[0]) <= slf._argflag['trace']['slits']['polyorder']+2:
#           lfail = np.append(lfail,i-lmin)
        ...
        lnmbrarr[i-lmin] = -i
        #lcoeff[:, i-lmin] = arutils.func_fit(tlfitx, tlfity, slf._argflag['trace']['slits']['function'],
        #                                     slf._argflag['trace']['slits']['polyorder'], minv=minvf, maxv=maxvf)
...
#		plt.clf()
    ...
    rcoeff = np.zeros((1+slf._argflag['trace']['slits']['polyorder'], rmax-rmin+1))
...
#			rfail = np.append(rfail, i-rmin)
        ...
        tlfitx = plxbin[w]
        tlfity = plybin[w]
        #rcoeff[:, i-rmin] = arutils.func_fit(tlfitx, tlfity, slf._argflag['trace']['slits']['function'],
        #                                     slf._argflag['trace']['slits']['polyorder'], minv=minvf, maxv=maxvf)
    ...
    if (lmax+1-lmin == 1) and (rmax+1-rmin == 1):
        ...
        lcenint[:, 0] = arutils.func_val(lcoeff[:, 0], xint, slf._argflag['trace']['slits']['function'],
                                         minv=minvf, maxv=maxvf)
        rcenint[:, 0] = arutils.func_val(rcoeff[:, 0], xint, slf._argflag['trace']['slits']['function'],
                                         minv=minvf, maxv=maxvf)
    ...
    lv = (arutils.func_val(lcoeff[:, lval-lmin], xv, slf._argflag['trace']['slits']['function'],
    ...
    """
    lvp = (arutils.func_val(lcoeff[:,lval+1-lmin],xv,slf._argflag['trace']['slits']['function'],min=minvf,max=maxvf)+0.5).astype(np.int)
    ...
    """
    if mnvalp > mnvalm:
        lvp = (arutils.func_val(lcoeff[:, lval+1-lmin], xv, slf._argflag['trace']['slits']['function'],
                                minv=minvf, maxv=maxvf)+0.5).astype(np.int)
...
#	rva = arutils.func_val(rcoeff[:,0],midval,slf._argflag['trace']['slits']['function'],min=xvmn,max=xvmx)
#	for i in range(1,rmax-rmin+1):
#		rvb = arutils.func_val(rcoeff[:,i],midval,slf._argflag['trace']['slits']['function'],min=xvmn,max=xvmx)
#		if rvb > lv and rva < lv:
#			lvp = arutils.func_val(lcoeff[:,500-lmin],midval*2.0,slf._argflag['trace']['slits']['function'],min=xvmn,max=xvmx)
#			lvm = arutils.func_val(lcoeff[:,500-lmin],0.0,slf._argflag['trace']['slits']['function'],min=xvmn,max=xvmx)
#			rvap = arutils.func_val(rcoeff[:,i-1],midval*2.0,slf._argflag['trace']['slits']['function'],min=xvmn,max=xvmx)
#			rvam = arutils.func_val(rcoeff[:,i-1],0.0,slf._argflag['trace']['slits']['function'],min=xvmn,max=xvmx)
#			rvbp = arutils.func_val(rcoeff[:,i],midval*2.0,slf._argflag['trace']['slits']['function'],min=xvmn,max=xvmx)
#			rvbm = arutils.func_val(rcoeff[:,i],0.0,slf._argflag['trace']['slits']['function'],min=xvmn,max=xvmx)
...
#			ymga += np.array([arutils.func_val(rcoeff[:,i-1],xv,slf._argflag['trace']['slits']['function'],min=xvmn,max=xvmx)])
#			ymgb += np.array([arutils.func_val(rcoeff[:,i],xv,slf._argflag['trace']['slits']['function'],min=xvmn,max=xvmx)])
...
#			rva = rvb
    ...
    rsub = int(round(rsub))
    if lmin < rmin-rsub:
        esub = lmin - (slf._argflag['trace']['slits']['pca']['extrapolate']['neg']+1)
    else:
        esub = (rmin-rsub) - (slf._argflag['trace']['slits']['pca']['extrapolate']['neg']+1)
    ...
    nmord = slf._argflag['trace']['slits']['polyorder']+1
    if armin != almin:
        if armin < almin:
    ...
    lcent = arutils.func_val(lcoeff[:,-lunq[lg][::-1]-1-slf._argflag['trace']['slits']['pca']['extrapolate']['neg']], xv,
                             slf._argflag['trace']['slits']['function'], minv=minvf, maxv=maxvf)
    rcent = arutils.func_val(rcoeff[:,runq[rg]-1-slf._argflag['trace']['slits']['pca']['extrapolate']['neg']], xv,
                             slf._argflag['trace']['slits']['function'], minv=minvf, maxv=maxvf)
    slitcen = 0.5*(lcent+rcent).T
    ##############
    if slf._argflag['trace']['slits']['pca']['type'] == 'order':
        ...
        totord = ordsnd[-1]+slf._argflag['trace']['slits']['pca']['extrapolate']['pos']
        ...
        ofit = slf._argflag['trace']['slits']['pca']
        lnpc = len(ofit)-1
        msgs.work("May need to do a check here to make sure ofit is reasonable")
        coeffs = arutils.func_fit(xv, slitcen, slf._argflag['trace']['slits']['function'],
                                  slf._argflag['trace']['slits']['polyorder'], minv=minvf, maxv=maxvf)
        ...
        fitted, outpar = arpca.basis(xcen, slitcen, coeffs, lnpc, ofit, x0in=ordsnd, mask=maskord,
                                     skipx0=False, function=slf._argflag['trace']['slits']['function'])
        arpca.pc_plot(slf, outpar, ofit, pcadesc=pcadesc)
        # Extrapolate the remaining orders requested
        orders = 1.0+np.arange(totord)
        extrap_cent, outpar = arpca.extrapolate(outpar, orders, function=slf._argflag['trace']['slits']['function'])
        ...
        diff_coeff, diff_fit = arutils.polyfitter2d(rcent-lcent, mask=maskord,
                                                    order=slf._argflag['trace']['slits']['diffpolyorder'])
        ...
        ydet = np.append(ydet, 1.0+ydetd*np.arange(1.0, 1.0+slf._argflag['trace']['slits']['pca']['extrapolate']['pos']))
    ...
    elif slf._argflag['trace']['slits']['pca']['type'] == 'pixel':
        maskord = np.where((np.all(lcoeff, axis=0) == False) | (np.all(rcoeff, axis=0) == False))[0]
        ...
        tcoeff = np.ones((slf._argflag['trace']['slits']['polyorder']+1, binarr.shape[1]))
        ...
        # trace values
        trcval = arutils.func_val(tcoeff, xv, slf._argflag['trace']['slits']['function'],
                                  minv=minvf, maxv=maxvf).T
        msgs.work("May need to do a check here to make sure ofit is reasonable")
        ofit = slf._argflag['trace']['slits']['pca']['params']
        lnpc = len(ofit)-1
...
def model_tilt(slf, det, msarc, censpec=None, maskval=-999999.9,
    ...
    if slf._argflag['trace']['slits']['tilts']['method']'.lower() == "zero":
    ...
    fitxy = [slf._argflag['trace']['slits']['tilts']['order'], 1]
    ...
    if slf._argflag['trace']['slits']['idsonly']:
    ...
    if slf._argflag['trace']['slits']['tilts']['method'].lower() == "pca":
        nitertilts = 2
    else:
        nitertilts = 1
    ...
    for tt in range(nitertilts):
        ...
        tcoeff = np.ones((slf._argflag['trace']['slits']['tilts']['order']+1, msarc.shape[0]))
        ...
        for j in range(arcdet.size):
            ...
            if slf._argflag['trace']['slits']['tilts']['method'].lower() == "spca":
            ...
            wmsk, mcoeff = arutils.robust_polyfit(xtfit[wmask], yfit,
                                                  slf._argflag['trace']['slits']['tilts']['order'],
                                                  function=slf._argflag['trace']['slits']['function'],
            ...
            if slf._argflag['trace']['slits']['tilts']['method'].lower() == "spline":
                mtilt[xint:xint+2*sz+1, j] = model/(msarc.shape[0]-1.0)
            elif slf._argflag['trace']['slits']['tilts']['method'].lower() == "interp":
        ...
        # Tilt values
        tiltval = arutils.func_val(tcoeff, xv, slf._argflag['trace']['slits']['function'],
                                   minv=0.0, maxv=msarc.shape[1]-1.0).T
        msgs.work("May need to do a check here to make sure ofit is reasonable")
        ofit = slf._argflag['trace']['slits']['tilts']['params']
        lnpc = len(ofit)-1
        ...
        if np.sum(1.0-extrap_row) > ofit[0]+1:
            ...
            extrap_tilt, outpar = arpca.extrapolate(outpar, orders, function=slf._argflag['trace']['slits']['function'])
        ...
        if slf._argflag['trace']['slits']['tilts']['method'].lower() == "interp":
        ...
        elif slf._argflag['trace']['slits']['tilts']['method'].lower() == "spline":
            ...
            if msgs._debug['tilts']:
                ...
                plt.show()
                debugger.set_trace()
        elif slf._argflag['trace']['slits']['tilts']['method'].lower() == "spca":
            # Slit position
            xspl = np.linspace(0.0, 1.0, msarc.shape[1])
            ...
            if False:
                ...
                plt.show()
                debugger.set_trace()
        elif slf._argflag['trace']['slits']['tilts']['method'].lower() == "pca":
...
def trace_tilt(slf, msarc, prefix="", tltprefix="", trcprefix=""):
    ...
    if slf._argflag['trace']['slits']['tilts']['method'] == 'zero':
...
#	tilts = np.arctan(-1.0/derv)*180.0/np.pi
    derv = -derv
    if slf._argflag['trace']['slits']['tilts']['method'] == 'perp':
        tilts = derv
...
#	plt.show()
    if slf._argflag['trace']['slits']['tilts']['method'] in ['perp','zero']:
        ...
        for i in range(maskorder.size):
            ...
            nm += 1
    elif slf._argflag['trace']['slits']['tilts']['method'] == 'fit1D':
...
#					tcoeff = np.polynomial.polynomial.polyfit(xtfit[wmask],ytfit[wmask],1)
                null, tcoeff = arutils.robust_polyfit(xtfit[wmask], ytfit[wmask], slf._argflag['trace']['slits']['tilts']['disporder'], sigma=2.0)
                #tcoeff = np.polynomial.polynomial.polyfit(xtfit[wmask],ytfit[wmask],1)
                # Save the tilt angle
        ...
        tcoeff = np.ones((slf._argflag['trace']['slits']['tilts']['disporder']+1,tiltang.shape[1]))
        ...
        for o in range(maskorder.size):
            ...
            if np.size(w[0]) <= slf._argflag['trace']['slits']['tilts']['disporder']+2:
                extrap_ord[o] = 1.0
                maskord = np.append(maskord,o)
            else:
                null, tempc = arutils.robust_polyfit(centval[:,o][w],tiltang[:,o][w], slf._argflag['trace']['slits']['tilts']['disporder'], function=slf._argflag['trace']['slits']['function'],sigma=2.0,
                                                     minv=0.0, maxv=msarc.shape[slf._dispaxis]-1)
                tcoeff[:,o] = tempc
#				tcoeff[:,o] = arutils.func_fit(centval[:,o][w],tiltang[:,o][w],slf._argflag['trace']['slits']['function'],slf._argflag['trace']['slits']['tilts']['disporder'],min=0.0,max=msarc.shape[slf._dispaxis]-1)
#				plt.clf()
#				plt.plot(centval[:,o][w],tiltang[:,o][w],'bx')
#				xmod = np.linspace(np.min(centval[:,o][w]),np.max(centval[:,o][w]),1000)
#				ymod = arutils.func_val(tcoeff[:,o],xmod,slf._argflag['trace']['slits']['function'],min=0.0,max=msarc.shape[slf._dispaxis]-1)
#				plt.plot(xmod,ymod,'r-')
#		plt.show()
        maskord.sort()
        xv = np.arange(msarc.shape[slf._dispaxis])
        tiltval = arutils.func_val(tcoeff,xv,slf._argflag['trace']['slits']['function'], minv=0.0,
        ...
        if np.sum(1.0-extrap_ord) > ofit[0]+1: # Only do a PCA if there are enough good orders
            ...
            fitted, outpar = arpca.basis(xcen,tiltval,tcoeff,lnpc,ofit,x0in=ordsnd,mask=maskord,skipx0=True,function=slf._argflag['trace']['slits']['function'])
            # If the PCA worked OK, do the following
            msgs.work("Should something be done here inbetween the two basis calls?")
            fitted, outpar = arpca.basis(xcen,tiltval,tcoeff,lnpc,ofit,x0in=ordsnd,mask=maskord,skipx0=False,function=slf._argflag['trace']['slits']['function'])
            ...
            extrap_tilt, outpar = arpca.extrapolate(outpar, orders, msgs, function=slf._argflag['trace']['slits']['function'])
            tilts = extrap_tilt
            ...
            if np.size(xtiltfit) > slf._argflag['trace']['slits']['tilts']['disporder']+2:
                tcoeff = arutils.func_fit(xtiltfit,ytiltfit,slf._argflag['trace']['slits']['function'],slf._argflag['trace']['slits']['tilts']['disporder'],
                                          minv=0.0, maxv=msarc.shape[slf._dispaxis]-1)
                tiltval = arutils.func_val(tcoeff,xv,slf._argflag['trace']['slits']['function'], minv=0.0,
    ...
    elif slf._argflag['trace']['slits']['tilts']['method'] == 'fit2D':
...
#					angle, error, fail = arfitbase.fit_tilt(sqrdata, pos, derv[pixt[j],i])
        ...
        for o in range(tiltang.shape[1]):
            ...
            if np.size(w[0]) <= slf._argflag['trace']['slits']['tilts']['disporder']+2:
                ogd[o] = 0.0
            tcoeff[:,o] = arutils.func_fit(arcdet[:,o][w],tiltang[:,o][w],slf._argflag['trace']['slits']['function'],slf._argflag['trace']['slits']['tilts']['disporder'],
                                           minv=0.0, maxv=msarc.shape[slf._dispaxis]-1)
        gd = np.where(ogd==1.0)[0]
        xv = np.arange(msarc.shape[slf._dispaxis])
        tiltval = arutils.func_val(tcoeff[:,gd],xv,slf._argflag['trace']['slits']['function'], minv=0.0,
                                   maxv=msarc.shape[slf._dispaxis]-1).T
        # Perform a PCA on the tilts
        ...
        coeffs = arutils.func_fit(xv,tiltval,slf._argflag['trace']['slits']['function'],slf._argflag['trace']['slits']['tilts']['disporder'],
                                  minv=0.0, maxv=msarc.shape[slf._dispaxis])
        xcen = xv[:,np.newaxis].repeat(gd.size,axis=1)
        fitted, outpar = arpca.basis(xcen,tiltval,coeffs,lnpc,ofit,x0in=gd+1.0,skipx0=True,function=slf._argflag['trace']['slits']['function'])
        # If the PCA worked OK, do the following
        msgs.work("Should something be done here inbetween the two basis calls?")
        fitted, outpar = arpca.basis(xcen,tiltval,coeffs,lnpc,ofit,x0in=gd+1.0,skipx0=False,function=slf._argflag['trace']['slits']['function'])
        arpca.pc_plot(outpar, ofit, plotsdir=slf._argflag['run']['directory']['qa'], pcatype="tilts", prefix=prefix)
        # Extrapolate the remaining orders requested
    ...
    elif slf._argflag['trace']['slits']['tilts']['method'] == 'trace':
...
#		tiltang[w] = np.arctan(derv[w])*180.0/np.pi
        #tiltang[w] = derv[w]
        msgs.info("Fitting tilt angles")
        tcoeff = np.zeros((slf._argflag['trace']['slits']['tilts']['disporder']+1,maskorder.size))
...
#		o = 0
        for ow in range(maskorder.size):
            ...
            if np.size(w[0]) <= slf._argflag['trace']['slits']['tilts']['disporder']+2:
                ...
                tcoeff[:,ow] = arutils.func_fit(centval[:,ow][w],tiltang[:,ow][w],slf._argflag['trace']['slits']['function'],slf._argflag['trace']['slits']['tilts']['disporder'],
                                                minv=0.0, maxv=msarc.shape[slf._dispaxis]-1)
#				o += 1
        xv = np.arange(msarc.shape[slf._dispaxis])
        tiltval = arutils.func_val(tcoeff,xv,slf._argflag['trace']['slits']['function'], minv=0.0,
        ...
        for o in range(tiltang.shape[1]):
            ...
            if np.size(w[0]) <= slf._argflag['trace']['slits']['tilts']['disporder']+2:
                ogd[o] = 0.0
                continue
            tcoeff[:,o] = arutils.func_fit(arcdet[:,o][w],tiltang[:,o][w],slf._argflag['trace']['slits']['function'],slf._argflag['trace']['slits']['tilts']['disporder'],min=0.0,max=msarc.shape[slf._dispaxis]-1)
        gd = np.where(ogd==1.0)[0]
        xv = np.arange(msarc.shape[slf._dispaxis])
        tiltval = arutils.func_val(tcoeff[:,gd],xv,slf._argflag['trace']['slits']['function'],min=0.0,max=msarc.shape[slf._dispaxis]-1).T
        # Perform a PCA on the tilts
        msgs.info("Performing a PCA on the tilt angles")
        ofit = slf._argflag['trace']['slits']['tilts']['params']
        lnpc = len(ofit)-1
        msgs.bug("May need to do a check here to make sure tilt ofit is reasonable")
        coeffs = arutils.func_fit(xv,tiltval,slf._argflag['trace']['slits']['function'],slf._argflag['trace']['slits']['tilts']['disporder'],min=0.0,max=msarc.shape[slf._dispaxis])
        xcen = xv[:,np.newaxis].repeat(gd.size,axis=1)
        fitted, outpar = arpca.basis(xcen,tiltval,coeffs,lnpc,ofit,x0in=gd+1.0,skipx0=True,function=slf._argflag['trace']['slits']['function'])
        # If the PCA worked OK, do the following
        ...
        msgs.warn("I don't know how to deal with '{0:s}' tilts".format(slf._argflag['trace']['slits']['tilts']['method']))
        msgs.info("Assuming there is no tilt")
        centval=-999999.9*np.ones((arcdet.shape[0],maskorder.size))
        nm=0
        for i in range(maskorder.size):
