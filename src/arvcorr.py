...
def helio_corr(slf, idx):
    ...
    date = slf._fitsdict["date"][idx]
    if (len(date)>10):
        # The UT must be appended to the date
        dspl = date.split("T")
        ddspl = dspl[0].split("-")
        year  = float(ddspl[0])
        month = float(ddspl[1])
        ...
        month = float(dspl[1])
        day   = float(dspl[2])
        # Now get the UTC for the *start* of the observation
        try:
            utc = slf._fitsdict["utc"][idx]
        except:
            msgs.error("Could not determine UTC of the observation"+msgs.newline()+"A heliocentric velocity correction cannot be performed")
        ddspl = utc.split(":")
        hour  = float(dspl[0])
        min   = float(dspl[1])
        sec   = float(dspl[2])
    ...
    vhel = vhelio(hdr_jd, hdr_year, hdr_month, hdr_day, hdr_ut, hdr_exptime, hdr_ra, hdr_dec, hdr_equ, hdr_lat, hdr_lon, hdr_alt)
    msgs.info("Heliocentric velocity correction = {0:+.4f} km/s for file:".format(vhel)+msgs.newline()+slf._fitsdict["filename"][idx])
    w = np.where(slf._waveids==-999999.9)
    slf._waveids += slf._waveids*vhel/299792.458
    slf._waveids[w] = -999999.9
    return slf._waveids

def vhelio(hdr_jd, hdr_year, hdr_month, hdr_day, hdr_ut, hdr_exptime, hdr_ra, hdr_dec, hdr_equ, hdr_lat, hdr_lon, hdr_alt):
    year, month, day, ut = jd_to_date(hdr_jd)
    msgs.work("Check the year, month, date and UT just calculated match the header")
    # If it doesn't match, just use the header information
    #if (hdr_year != year) or (hdr_month != month) or (hdr_day != day):
    #	msgs.warn("Observation date from header does not match the date calculated"+msgs.newline()+"from the MJD in the header")
    #	msgs.info("Adopting the observation date from header")
    #elif abs(ut-hdr_ut)>uttol/60.0:
    #	if hdr_ut < uttol/60.0 and abs(ut+hdr_ut-24.0)<UTTOL/60.0:
...
def vorbit(ra, dec, epoch):
    ...
    manom   = manom - int(manom/360.0)*360.0
    lperi   = lperi - int(lperi/360.0)*360.0
    # Convert to radians
    ...
    tanom = manom + (2.0*eccen-0.25*eccencu)*np.sin(manom) + 1.25*eccensq*np.sin(2.0*manom) + (13.0/12.0)*eccencu*np.sin(3.0*manom)
...
def vrotate(ra, dec, epoch, lat, lon, alt):
    ...
    r  = 6378160.0*(0.998327073+0.00167643800*np.cos(2.0*latd)-0.00000351*np.cos(4.0*latd)+0.000000008*np.cos(6.0*latd))+alt
