...
This doc goes through a full run of PypeIt on one of each of our example
LDT/RIMAS datasets (specifically the ``Vph300_YJ`` / ``Vph300_HK`` and
``Grism_YJ`` / ``Grism_HK`` datasets).  If you're having trouble reducing
your data, we encourage you to try going through this tutorial using one of the
example datasets first.  See :ref:`here <dev-suite>` to find the example
dataset, please join our `PypeIt Users Slack <https://pypeit-users.slack.com>`__
using `this invitation link <invite_>`_ to ask for help, and/or `Submit an
issue`_ to Github if you find a bug!

This document is divided into sections outlining the single-order spectra
obstained using the VPH gratings and the echelle grism.  There are subtle
differences in using the pipeline with the different data sets, so please
direct your attention to the appropriate section for your data.
...
.. code-block:: bash
    ...
    pypeit_setup -r <path-to-DevSuite>/RAW_DATA/ldt_rimas/Vph300_HK/ -s ldt_rimas_vph -c all

where the ``-r`` argument should be replaced by your local directory.  RIMAS
