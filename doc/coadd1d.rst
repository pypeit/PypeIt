...
The :ref:`pypeit_coadd_1dspec` script requires an
...
output file 
and/or modify default parameters for the script.
...
See `Parameters`_ below for common choices.
...
It is possible to provide only a single row (it must be the first row)
for the ``obj_id``.  This will then be used for all of the :doc:`out_spec1D` files input.
...
The primary script is called :ref:`pypeit_coadd_1dspec` 
which takes a `coadd1d File`_ to guide the process.
