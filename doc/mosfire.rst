...
objects in the slits. However, a better procedure would be to let ``PypeIt`` computes the offset using
a bright object in one of the slits in the slit-mask. To do so, the uses should provide in the :doc:`pypeit_file`
...
Besides these two slits and the central alignment box, the rest of the mask is filled with random slits.
