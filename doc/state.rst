...
- **During a run.** ``run_pypeit`` updates the state as it goes: it marks a step
  ...
  the run's progress.
- **Without running.** The ``pypeit_status`` script derives the same state
  *without* performing any processing — it instantiates PypeIt in
  ``calib_only`` mode, checks what calibrations exist, and prints a status
  table.  It also writes a human-readable ``<pypeit_root>.status.log``.
...
.. warning::
   ...
   **Do not edit the** ``_state.json`` **file by hand.**  It is generated and
   ...
   and can make the file unreadable.  To refresh the state, re-run the
   reduction (or ``pypeit_status``).
