...
for DEIMOS an it is set as the default one.
...
is performed again for each snippet to get a better estimate of the shift+stretch in wavelengths that is then
applied to the input spectrum. The wavelength solution is determined by using arc lines detected in the input
spectrum that match the wavelengths from the :ref:`wave_calib:Line Lists` (stored as as ASCII tables
...
- **fwhm** is an assumed value of the arc lines FWHM, i.e., spectral resolution, in unbinned pixels. The default for
  ...
  keyword **fwhm_fromlines** should take care of this.
...
- Inspecting the Quality Assurance (QA) plot generated by ``PypeIt`` saved in PNG files.
...
``PypeIt`` meets these requirements as demonstrated by the tests performed by th ``PyPeIt`` team and described below.

Description
^^^^^^^^^^^

We performed the wavelength calibration on a large sample of 36 DEIMOS datasets spanning a wide range of wavelength,
gratings, central wavelengths, and slit width:
...
Including six datasets using LVM slitmask, which is composed of 5 slits with different widths (0.7", 0.8", 1.",
1.2", 1.5"). these datasets were taken with the following gratings (and central wavelengths): 830G (8400 |AA|),
600ZD (7100 |AA|), 830G (7900 |AA|), 1200G (7800 |AA|), 1200B (5450 |AA|), 1200B (5200 |AA|).

Results
^^^^^^^

Out of the **4072 slits** that have been tested, for only **37 (0.9%)** the wavelength calibration failed or
was wrong. From our visual inspection, we found that the success of the wavelength calibration is mostly
dictated by the quality of the arc spectra. In particular, artifacts, noisy spectra, ghosts, and strong
...
provided by ``PypeIt`` user Kevin McKinnon).
