...
``optimal_fwhm``      int, float                                         ..       ..                               FWHM in arcseconds used for optimal extraction (not implemented for echelle). If this parameter is ``None`` (default), `FindObj` estimates the FWHM for each detected object (see FindObjPar).                                                                                               
...
``force_extract``    bool        ..       False    Force extraction of undetected objects at the location expected from the slitmask design.                                                                                                                                     
``force_fwhm``       int, float  ..       ..       Initial FWHM in arcsec used for the extraction of undetected objects.During extraction, PypeIt will still try to fit a profile to improve this value. To keep this value, instead, set also ``optimal_fwhm`` in ExtractionPar.
``mask_median_off``  int, float  ..       0.0      Median offset in pixels of slitmask from expected position.This parameter is only used during the forced extraction of undetected objects.                                                                                    
