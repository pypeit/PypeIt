...
class ScatteredLightPar(parset.ParSet):
    ...
    valid_scattlight_methods = ['model', 'frame', 'archive', 'gaps']
    """
    ...
    parameters = {
        'method': parset.set_parameter_definition(
            ...
            descr=(
                ...
                'file).  Select "frame" to use each individual frame to determine the scattered '
                'light directly from that frame.  Select "archive" to use an archival model '
                'parameter solution for the scattered light (this option is not currently '
                'available for all spectrographs).'
            ),
        ),
        'finecorr_method': parset.set_parameter_definition(
            ...
            descr=(
                ...
                'the scattered light in each row, based on the pixels that are not on slits (see '
                'also, "finecorr_pad").'
            ),
        ),
...
class ProcessImagesPar(parset.ParSet):
    ...
    parameters = {
        ...
        'use_overscan': parset.set_parameter_definition(
            ...
            default=True,
            descr=(
                'Subtract off the overscan.  Detector *must* have a valid overscan region defined '
        ...
        'overscan_par': parset.set_parameter_definition(
            ...
            default=[5, 65],
            descr=(
                'Parameters for the overscan subtraction.  For "chebyshev" or "polynomial", set '
                '``overscan_par = order``; for "savgol", set ``overscan_par = order, window '
                'size``; for "median", set ``overscan_par = None`` or omit the keyword.'
            ),
        ),
        'correct_nonlinear': parset.set_parameter_definition(
            dtype=list,
            descr=(
                'Correct for non-linear response of the detector.  If None, no correction is '
                'performed. If a list, then the list should be the non-linear correction '
                r'parameter (:math:`\alpha`), where the functional form is given by :math:`C_t = '
                r'C_m (1 + \alpha\ C_m)`, with :math:`C_t` and :math:`C_m` the true and measured '
                'counts. This parameter is usually hard-coded for a given spectrograph, and '
                'should otherwise be left as None.'
            ),
        ),
        ...
        'use_pattern': parset.set_parameter_definition(
            ...
            default=False,
            descr=(
                'Subtract off a detector pattern. This pattern is assumed to be sinusoidal along '
                'one direction, with a frequency that is constant across the detector.'
            ),
        ),
        'subtract_continuum': parset.set_parameter_definition(
            ...
            default=False,
            descr=(
                'Subtract off the continuum level from an image. This parameter should only be '
                'set to True to combine arcs with multiple different lamps. For all other cases, '
                'this parameter should probably be False.'
            ),
        ),
        'subtract_scattlight': parset.set_parameter_definition(
            ...
            default=False,
            descr=(
                'Subtract off the scattered light from an image. This parameter should only be '
                'set to True for spectrographs that have dedicated methods to subtract scattered '
                'light. For all other cases, this parameter should be False.'
            ),
        ),
        ...
        'empirical_rn': parset.set_parameter_definition(
            ...
            default=False,
            descr=(
                'If True, use the standard deviation in the overscan region to measure an '
                'empirical readnoise to use in the noise model.'
            ),
        ),
        'shot_noise': parset.set_parameter_definition(
            ...
            default=True,
            descr=(
                'Use the bias- and dark-subtracted image to calculate and include electron count '
                'shot noise in the image processing error budget'
            ),
        ),
        'noise_floor': parset.set_parameter_definition(
            ...
            default=0.0,
            descr=(
                'Impose a noise floor by adding the provided fraction of the bias- and '
                'dark-subtracted electron counts to the error budget.  E.g., a value of 0.01 '
                'means that the S/N of the counts in the image will never be greater than 100.'
            ),
        ),
        ...
        'use_specillum': parset.set_parameter_definition(
            ...
            default=False,
            descr=(
                'Use the relative spectral illumination profiles to correct the spectral '
                'illumination profile of each slit. This is primarily used for slicer IFUs.  To '
                'use this, you must set ``slit_illum_relative=True`` in the ``flatfield`` '
                'parameter set!'
            ),
        ),
        'skip_write_2d': parset.set_parameter_definition(
            ...
            default=False,
            descr=(
                'Skip writing the 2D spectrum for science frames.  WARNING: This option should '
                'only be considered for reducing the volume of output data when processing large '
                'numbers of frames and only after ensuring the quality of the resulting '
                'reductions.'
            ),
        ),
        ...
        'spat_flexure_sigdetect': parset.set_parameter_definition(
            ...
            default=5.0,
            descr=(
                'Sigma threshold above fluctuations in the Sobel-filtered significance image, '
                'used for finding slit edges in the spectral image, for which the spatial flexure '
                'is computed.'
            ),
        ),
        'spat_flexure_vrange': parset.set_parameter_definition(
            dtype=tuple,
            descr=(
                'This parameter is used when generating the QA plot for the spatial flexure. It '
                'sets the data range (vmin,vmax) used by the colormap when showing the spectral '
                'image. If None, the range is set automatically.'
            ),
        ),
        ...
        'n_lohi': parset.set_parameter_definition(
            ...
            default=[0, 0],
            descr=(
                'Number of pixels to reject at the lowest and highest ends of the distribution; '
                'i.e., ``n_lohi = low, high``.  Use None for no limit.'
            ),
        ),
        ...
        'grow': parset.set_parameter_definition(
            ...
            default=1.5,
            descr=(
                'Factor by which to expand regions with cosmic rays detected by the LA cosmics '
                'routine.'
            ),
        ),
        ...
        'cr_median_width': parset.set_parameter_definition(
            dtype=int,
            default=0,
            descr=(
                'Width (in pixels, along axis 1) of the row-local median filter applied by '
                'build_crmask before running L.A.Cosmic on the residual.  Useful for line-rich '
                'frames with small tilts (e.g. arcs, tilts) where the standard 3x3 Laplacian '
                'misses the bodies of extended trails.  Only active when ``mask_cr=True``.  Set '
                'to 0 (default) to disable; set to an odd integer (e.g. 31) to enable.'
            ),
        ),
    }
...
class FrameGroupPar(parset.ParSet):
    ...
    parameters = {
        'exprng': parset.set_parameter_definition(
            ...
            default=[None, None],
            descr=(
                'Used in identifying frames of this type.  This sets the minimum and maximum '
                'allowed exposure times.  There must be two items in the list.  Use None to '
                'indicate no limit; i.e., to select exposures with any time greater than 30 sec, '
                'use ``exprng = [30, None]``.'
            ),
        ),
    }
    ...
    parameters = {
        ...
        'spat_samp': parset.set_parameter_definition(
            ...
            descr=(
                ...
                'filter in pixels used to determine the slit illumination function, and thus sets '
                'the minimum scale on which the illumination function will have features.'
            ),
        ),
        'pixelflat_min_wave': parset.set_parameter_definition(
            dtype=[int, float],
            descr=(
                'All values of the normalized pixel flat are set to 1 for wavelengths below this '
                'value.'
            ),
        ),
        'pixelflat_max_wave': parset.set_parameter_definition(
            dtype=[int, float],
            descr=(
                'All values of the normalized pixel flat are set to 1 for wavelengths above this '
                'value.'
            ),
        ),
        'tweak_slits': parset.set_parameter_definition(
            ...
            default=True,
            descr=(
                'Use the illumination flat field to tweak the slit edges. This will work even if '
                'illumflatten is set to False '
            ),
        ),
        'tweak_method': parset.set_parameter_definition(
            ...
            descr=(
                'Method used to tweak the slit edges (when "tweak_slits" is set to True). Options '
                f'include: {", ".join(valid_tweak_methods)}. The "threshold" method determines '
                'when the left and right slit edges fall below a threshold relative to the peak '
                'illumination. The "gradient" method determines where the gradient is the highest '
                'at the left and right slit edges. This method performs better when there is '
        ...
        'tweak_slits_thresh': parset.set_parameter_definition(
            ...
            default=0.93,
            descr=(
                'If ``tweak_slits`` is True, this sets the illumination function threshold used '
                'to tweak the slit boundaries based on the illumination flat. It should be a '
        ...
        'rej_sticky': parset.set_parameter_definition(
            ...
            default=False,
            descr=(
                'Propagate the rejected pixels through the stages of the flat-field fitting (i.e, '
                'from the spectral fit, to the spatial fit, and finally to the 2D residual fit).  '
                'If False, pixels rejected in each stage are included in each subsequent stage.'
            ),
        ),
        'slit_trim': parset.set_parameter_definition(
            ...
            default=3.0,
            descr=(
                'The number of pixels to trim each side of the slit when selecting pixels to use '
                'for fitting the spectral response function.  Single values are used for both '
                'slit edges; a two-tuple can be used to trim the left and right sides differently.'
            ),
        ),
        ...
        'slit_illum_finecorr': parset.set_parameter_definition(
            ...
            default=True,
            descr=(
                'If True, a fine correction to the spatial illumination profile will be '
                'performed. The fine correction is a low order 2D polynomial fit to account for a '
                'gradual change to the spatial illumination profile as a function of wavelength.'
            ),
        ),
        'slit_illum_relative': parset.set_parameter_definition(
            ...
            default=False,
            descr=(
                'Generate an image of the relative spectral illumination for a multi-slit setup.  '
                'If you set ``use_specillum = True`` for any of the frames that use the flatfield '
                'model, this *must* be set to True. Currently, this is only used for SlicerIFU '
                'reductions.'
            ),
        ),
        'illum_iter': parset.set_parameter_definition(
            ...
            descr=(
                ...
                'slit-illumination profile.  No rejection iterations are performed if 0.  '
                'WARNING: Functionality still being tested.'
            ),
        ),
        'illum_rej': parset.set_parameter_definition(
            ...
            default=5.0,
            descr=(
                'The sigma threshold used in the rejection iterations used to refine the '
                'slit-illumination profile.  Rejection iterations are only performed if '
                '``illum_iter > 0``.'
            ),
        ),
        ...
        'slit_illum_ref_idx': parset.set_parameter_definition(
            ...
            default=0,
            descr=(
                'The index of a reference slit (0-indexed) used for estimating the relative '
                'spectral sensitivity (or the relative blaze). This parameter is only used if '
        ...
        'slit_illum_smooth_npix': parset.set_parameter_definition(
            ...
            default=10,
            descr=(
                'The number of pixels used to determine smoothly varying relative weights is '
                'given by ``nspec/slit_illum_smooth_npix``, where nspec is the number of spectral '
                'pixels.'
            ),
        ),
        'fit_2d_det_response': parset.set_parameter_definition(
            ...
            default=False,
            descr=(
                'Set this variable to True if you want to compute and account for the detector '
                'response in the flatfield image. Note that ``detector response`` refers to pixel '
        ...
        'fiber_pixelflat': parset.set_parameter_definition(
            dtype=bool,
            default=False,
            descr=(
                'For the Fiber pypeline only: build a standard 2D pixel flat from the flat frames '
                '(via the normal FlatField machinery) instead of using a unity pixel flat.  The '
                'default (False) is appropriate for in-focus fiber flats that illuminate only a '
                'few pixels under each fiber (e.g. MMT Binospec), where a 2D pixel flat would '
                'imprint fiber-profile structure.  Set True for instruments whose flats '
                'illuminate the full detector (e.g. defocused fiber flats), so a meaningful '
                'detector-response pixel flat can be measured.'
            ),
        ),
    }
...
class FlexurePar(parset.ParSet):
    ...
    parameters = {
        'spec_method': parset.set_parameter_definition(
            ...
            options=valid_methods,
            descr=(
                'Method used to correct for flexure. Use skip for no correction.  If slitcen is '
                'used, the flexure correction is performed before the extraction of objects (not '
                f'recommended).  Options are: {", ".join(valid_methods)}'
            ),
        ),
        ...
        'spectrum': parset.set_parameter_definition(
            ...
            descr=(
                ...
                '``pypeit/data/sky_spec/`` for a list of available sky spectra. If ``model`` is '
                'used, a model sky spectrum will be generated using '
                ':func:`~pypeit.wavemodel.nearIR_modelsky` and the spectral resolution of the '
                'spectrum to be flexure corrected.'
            ),
        ),
        'excessive_shift': parset.set_parameter_definition(
            ...
            descr=(
                ...
                '``spec_maxshift``.  The options are: "crash" - Raise an error and halt the data '
                'reduction; "set_to_zero" - Set the flexure shift to zero and continue with the '
                'reduction; "continue" - Use the large flexure value whilst issuing a warning; '
                'and "use_median" - Use the median flexure shift among all the objects in the '
                'same slit (if more than one object is detected) or among all the other slits; if '
                'not available, the flexure correction will not be applied.'
            ),
        ),
    ...
    parameters = {
        ...
        'sigrej_exp': parset.set_parameter_definition(
            ...
            descr=(
                ...
                '``sigrej_exp*sigma`` above the median S/N. If None (the default), no rejection '
                'is performed. Currently, only available for multi-slit observations.'
            ),
        ),
        ...
        'dv': parset.set_parameter_definition(
            dtype=[int, float],
            descr=(
                'Dispersion in units of km/s in case you want to specify it in '
                ':func:`~pypeit.core.wavecal.wvutils.get_wave_grid` (for the "velocity" option), '
        ...
        'dwave': parset.set_parameter_definition(
            dtype=[int, float],
            descr=(
                'Dispersion in Angstroms in case you want to specify it in '
                ':func:`~pypeit.core.wavecal.wvutils.get_wave_grid` (for the "linear" option), '
                'otherwise a median value is computed from the data.'
            ),
        ),
        'dloglam': parset.set_parameter_definition(
            dtype=[int, float],
            descr=(
                'Dispersion in units of log10(wave) in case you want to specify it in '
                ':func:`~pypeit.core.wavecal.wvutils.get_wave_grid` (for the "velocity" or '
                '"log10" options), otherwise a median value is computed from the data.'
            ),
        ),
        ...
        'weight_method': parset.set_parameter_definition(
            ...
            descr=(
                ...
                'constant weights if rms_sn < 3.0, otherwise use wavelength dependent; "constant" '
                '- Constant weights based on rms_sn**2; "uniform" - Uniform weighting; '
        ...
        'filter': parset.set_parameter_definition(
            ...
            default='none',
            descr=(
                'Filter for scaling.  See flux_calib.load_fitler_file() for naming.  Ignore if '
                'none.'
            ),
        ),
    ...
    parameters = {
        ...
        'exclude_slits': parset.set_parameter_definition(
            ...
            descr=(
                ...
                'DET01:175,DET02:205 or MSC02:2234. This and ``only_slits`` are mutually '
                'exclusive. If both are provided, ``only_slits`` takes precedence.'
            ),
        ),
        'offsets': parset.set_parameter_definition(
            ...
            descr=(
                ...
                'matching (currently available for these :ref:`slitmask_info_instruments` only). '
                'If equal to ``header``, the dither offsets recorded in the header, when '
                'available, will be used.  If ``auto`` is chosen, PypeIt will try to compute the '
                'offsets using a reference object with the highest S/N, or using a list of object '
                'ids selected by the user (see ``user_obj_ids``).  If a list of offsets is '
                'provided, PypeIt will use it.'
            ),
        ),
        ...
        'weights': parset.set_parameter_definition(
            ...
            default='auto',
            descr=(
                'Mode for the weights used to coadd images. Options are: ``auto``, ``uniform``, '
                'or a list of weights. If a list of weights is provided, PypeIt will use it. If '
                '``uniform`` is used, uniform weights will be applied. If ``auto`` is used, '
                'PypeIt will try to compute the weights using a reference object with the highest '
                'S/N, or using a list of object ids selected by the user indicating a reference '
                'object in each exposure (see ``user_obj_ids``). If the reference object is not '
                'found, the code will use uniform weights. '
            ),
        ),
        'user_obj_ids': parset.set_parameter_definition(
            dtype=list,
            descr=(
                'List of unique object identifiers that the user wants to use to compute the '
                'weights and/or the offsets for coadding images. For longslit/multislit '
                'spectroscopy, provide the ``SPAT_PIXPOS_ID`` of the object in each of the '
                'exposures. For echelle spectroscopy, provide the ``ECH_FRACPOS_ID`` of the '
                'object in each exposure. These unique object identifiers can be found in the '
                'spec1d*.txt files for each exposure. See :ref:`spec-1d-output` for more info '
        ...
        'manual': parset.set_parameter_definition(
            ...
            descr=(
                ...
                '(1,2,3):82.4:608.1:3.``.  Note ``spat,spec`` are in the pixel coordinates of the '
                'pseudo-image generated by COADD2D; ``fwhm`` is in pixels, and ``boxcar_radius`` '
...
class CubeExtractionPar(parset.ParSet):
    """
    ...
    .. include:: ../include/parset_CubeExtractionPar.rst
    """

    valid_opt_prof_methods = ['user_gauss', 'fit_gauss', 'whitelight']
    ...
    parameters = {
        ...
        'whitelight_range': parset.set_parameter_definition(
            ...
            descr=(
                ...
                'automatically use a wavelength range that ensures all spaxels have the same '
                'wavelength coverage.'
            ),
        ),
        'fwhm': parset.set_parameter_definition(
            ...
            default=1.5,
            descr=(
                'FWHM of the PSF in arcseconds. Used to determine the degree of smoothing of the '
                'whitelight image, the kernel size for the initial object finding, and the bounds '
                'of the parameters for the 2D Gaussian fit. Note that if the ``opt_prof_method`` '
                'is set to "user_gauss", this parameter will also be used as the FWHM of the 2D '
                '(symmetric) Gaussian spatial profile for optimal extraction. Default is 1.5 '
                'arcseconds.'
            ),
        ),
        'snr_thresh': parset.set_parameter_definition(
            ...
            default=5.0,
            descr=(
                'The signal-to-noise ratio threshold to use when determining the initial object '
                'position in the whitelight image with '
                ':class:`~photutils.detection.DAOStarFinder` (this is the nsigma parameter in '
                ':func:`~pypeit.core.datacube.fitGaussian2D`)'
            ),
        ),
        'manual': parset.set_parameter_definition(
            dtype=str,
            descr=(
                'Manual extraction position for ``pypeit_extract_datacube``, in the format '
                '``x:y``, where x,y are the image coordinates read from Ginga or DS9.  In numpy '
                'terms, if the image has shape (ny, nx), a position (x, y) refers to image[y, '
                'x].  Manual extraction is currently only possible for a single object; a '
                'semi-colon-separated list of multiple x:y positions is not supported and will '
                'raise an error.'
            ),
        ),
        ...
        'opt_prof_method': parset.set_parameter_definition(
            dtype=str,
            default='fit_gauss',
            options=valid_opt_prof_methods,
            descr=(
                ...
                '``"whitelight"``: ``"fit_gauss"`` uses the (possibly asymmetric) 2D Gaussian fit '
                'to the whitelight image, which was used to determine the object position.  This '
                'creates a model using :func:`~pypeit.core.datacube.fitGaussian2D` but the offset '
                'is set to zero.  ``"user_gauss"`` uses a 2D symmetric Gaussian profile. The FWHM '
                'of the Gaussian is determined by the ``fwhm`` parameter, which was also used for '
                'the object finding.  Note that this assumes the spatial pixel (spaxel) sampling '
                'is the same in both x and y.  ``"whitelight"`` uses the whitelight image to '
                'determine a non-parametric spatial profile. The whitelight image is smoothed '
                'with a Gaussian kernel of width 0.5*sigma, where sigma is the standard deviation '
                '(fwhm/2.35) corresponding to the ``fwhm`` parameter.'
            ),
        ),
        'skysub_resid': parset.set_parameter_definition(
            dtype=bool,
            default=True,
            descr=(
                'If True, the residual sky in the cube will be subtracted before extraction. The '
                'residual sky in the whitelight image will also be subtracted.'
            ),
        ),
    }
    ...
    def validate(self):
        ...
        if self['manual'] is not None:
            if ';' in self['manual']:
                raise ValueError(
                    'Manual extraction parameters must specify a single object; only the x:y '
                    'format is currently supported, and a semi-colon-separated list of multiple '
                    'objects is not.'
                )
            _manual = self['manual'].split(':')
            if len(_manual) != 2:
                raise ValueError(
                    'When providing manual extraction parameters, only x and y can be provided, '
                    'and the format must be x:y (e.g. --manual 10.0:14.0). Only a single object '
                    'can currently be extracted at a time.'
                )
...
class CubePar(parset.ParSet):
    ...
    valid_alignment_methods = ['none', 'user', 'phase', 'cc', 'fit']

    parameters = {
        'extraction': parset.set_parameter_definition(
            dtype=CubeExtractionPar,
            default=CubeExtractionPar(),
            descr='Parameters for cube spectral extraction algorithms',
        ),
        ...
        'save_native': parset.set_parameter_definition(
            dtype=bool,
            default=False,
            descr=(
                'If set to True, PypeIt will write spec3d datacube files for each of the input '
                'spec2d files, at the native sampling of the instrument.'
            ),
        ),
        'save_separate': parset.set_parameter_definition(
            dtype=bool,
            default=False,
            descr=(
                'If set to True, PypeIt will write spec3d datacube files for each of the input '
                'spec2d files, at the final (combined) sampling of the instrument.'
            ),
        ),
        'alignment_method': parset.set_parameter_definition(
            dtype=str,
            default='phase',
            options=valid_alignment_methods,
            descr=(
                'Whitelight images of all input frames will be generated and spatially aligned '
                'using either a reference image (see ``reference_image``) or the whitelight image '
                'of the first spec2d listed in the coadd3d file. This parameter allows you to set '
                'the method used to spatially align the datacubes. The current allowed options '
                'include "none", "phase", "cc", "fit", and "user". Setting ``alignment_method = '
                'phase`` (the default) will use a phase cross-correlation method to determine the '
        ...
        'output_filename': parset.set_parameter_definition(
            ...
            default='',
            descr=(
                'If combining multiple frames, this string sets the output filename of the '
                'combined datacube. If ``combine=False``, the output filenames will be prefixed '
                'with ``spec3d_*``'
            ),
        ),
        'sensfile': parset.set_parameter_definition(
            dtype=str,
            descr=(
                'Filename of a sensitivity function to use to flux calibrate your datacube. The '
                'sensitivity function file will also be used to correct the relative scales of '
                'the slits.'
            ),
        ),
        'reference_image': parset.set_parameter_definition(
            dtype=str,
            descr=(
                'White light image of a previously combined datacube. The white light image will '
                'be used as a reference when calculating the offsets of the input spec2d files. '
                'Ideally, the reference image should have the same shape as the data to be '
                'combined (i.e. set the ``ra_min``, ``ra_max`` etc. params so they are identical '
        ...
        'save_whitelight': parset.set_parameter_definition(
            ...
            default=False,
            descr=(
                'Save a white light image of the combined datacube. The output filename will be '
                'given by the ``output_filename`` variable with a suffix "_whitelight". Note that '
                'the white light image collapses the flux along the wavelength axis, so some '
                'spaxels in the 2D white light image may have different wavelength ranges. To set '
                'the wavelength range, use the ``whitelight_range`` parameter. If '
                '``combine=False``, the individual spec3d files will have a suffix "_whitelight".'
            ),
        ),
        'whitelight_range': parset.set_parameter_definition(
            dtype=list,
            ...
            descr=(
                ...
                'wavelength to use. If either of these elements are None, PypeIt will '
                'automatically use a wavelength range that ensures all spaxels have the same '
                'wavelength coverage. Note, if you are using a ``reference_image`` to align all '
                'frames, it is preferable to use the same white light wavelength range for all '
                'white light images. For example, you may wish to use an emission line map to '
                'register two frames.'
            ),
        ),
        'method': parset.set_parameter_definition(
            ...
            descr=(
                ...
                'does not account for covariance between adjacent voxels. See also, '
                '``spec_subpixel`` and ``spat_subpixel``. (2) "ngp" (nearest grid point) - this '
                'algorithm is effectively a 3D histogram. Flux is conserved, voxels are not '
                'correlated, however this option suffers the same downsides as any histogram; the '
                'choice of bin sizes can change how the datacube appears. This algorithm takes '
                'each pixel on the spec2d frame and puts the flux of this pixel into one voxel in '
                'the datacube. Depending on the binning used, some voxels may be empty (zero '
                'flux) while a neighboring voxel might contain the flux from two spec2d pixels. '
                'Note that all spec2d pixels that contribute to the same voxel are inverse '
                'variance weighted (e.g. if two pixels have the same variance, the voxel would be '
                'assigned the average flux of the two pixels).'
            ),
        ),
        'spec_subpixel': parset.set_parameter_definition(
            ...
            descr=(
                ...
                'each detector pixel in the spectral direction. The total number of subpixels in '
                'each pixel is given by ``spec_subpixel`` x ``spat_subpixel``. The default option '
                'is to divide each spec2d pixel into 25 subpixels during datacube creation. See '
                'also, ``spat_subpixel`` and ``slice_subpixel``.'
            ),
        ),
        'spat_subpixel': parset.set_parameter_definition(
            ...
            descr=(
                ...
                'each detector pixel in the spatial direction. The total number of subpixels in '
                'each pixel is given by ``spec_subpixel`` x ``spat_subpixel``. The default option '
                'is to divide each spec2d pixel into 25 subpixels during datacube creation. See '
                'also, ``spec_subpixel`` and ``slice_subpixel``.'
            ),
        ),
        ...
        'spatial_delta': parset.set_parameter_definition(
            dtype=float,
            descr=(
                'The spatial size of each spaxel to use when generating the WCS (in arcsec). If '
                'None, the default is set by the spectrograph file.'
            ),
        ),
        'wave_delta': parset.set_parameter_definition(
            dtype=float,
            descr=(
                'The wavelength step to use when generating the WCS (in Angstroms). If None, the '
                'default is set by the wavelength solution.'
            ),
        ),
        ...
        'weights_init_obj_pos': parset.set_parameter_definition(
            dtype=str,
            descr=(
                ...
                'constrained to be within +/- fwhm/3 in x and y.  If not set, the position will '
                'be determined by running :class:`~photutils.detection.DAOStarFinder` on the '
                'image.  Formatting follows the manual extraction parameters convention, i.e. '
                'x:y.  The x,y values are the image coordinates read from Ginga or DS9.  In numpy '
                'terms, if the image has shape (ny, nx), a position (x, y) refers to image[y, x].'
            ),
        ),
        'sn_smooth_npix': parset.set_parameter_definition(
            dtype=[int, float],
...
class FluxCalibratePar(parset.ParSet):
    ...
    parameters = {
        'extrap_sens': parset.set_parameter_definition(
            ...
            default=False,
            descr=(
                'If False (default), the code will crash if one tries to use sensfunc at '
                'wavelengths outside its defined domain. By changing the '
                '``par["sensfunc"]["extrap_blu"]`` and ``par["sensfunc"]["extrap_red"]`` this '
                'domain can be extended. If True the code will blindly extrapolate.'
            ),
        ),
...
class SensfuncUVISPar(parset.ParSet):
    ...
    parameters = {
        ...
        'nresln': parset.set_parameter_definition(
            ...
            default=20,
            descr=(
                'Parameter governing the spacing of the bspline breakpoints in terms of number of '
                'resolution elements.'
            ),
        ),
...
class TelluricPar(parset.ParSet):
    ...
    parameters = {
        'telgridfile': parset.set_parameter_definition(
            dtype=str,
            descr=(
                'File with the telluric model spectra to use.  Generally, these do not need to be '
                'set; reasonable defaults are provided for each spectrograph.  Due to their size, '
                'the files are not included with the released pypeit package; instead the code '
                'downloads each file into your cache as needed.  If this parameter is set in your '
                'pypeit file, it can be the path to a local file (which must have the correct '
                'format), or it can be the name of the specific cache file to use (e.g., '
        ...
        'teltype': parset.set_parameter_definition(
            ...
            descr=(
                'Method used to evaluate telluric models.  Options are ``pca`` or ``grid``. The '
                '``grid`` option uses a fixed grid of pre-computed HITRAN+LBLRTM atmospheric '
                'transmission models for each observatory, whereas the ``pca`` option uses '
                'principal components of a larger model grid to compute an accurate '
                'pseudo-telluric model with a much lighter telgridfile.'
            ),
        ),
        'sn_clip': parset.set_parameter_definition(
            dtype=[int, float],
        ...
        'resln_guess': parset.set_parameter_definition(
            ...
            descr=(
                ...
                r':math:`\lambda/d\lambda`. The resolution is fit explicitly as part of the '
                'telluric model fitting, but this guess helps determine the bounds for the '
                'optimization (see ``resln_frac_bounds``). If not provided, the wavelength '
                'sampling of your spectrum will be used and the resolution calculated using a '
                'typical sampling of 3 spectral pixels per resolution element.'
            ),
        ),
        'resln_frac_bounds': parset.set_parameter_definition(
            ...
            default=(0.6, 1.4),
            descr=(
                'Bounds for the resolution fit optimization which is part of the telluric model.  '
                'This range is in units of ``resln_guess``, so the default would bound the '
                'spectral resolution fit to be within the range ``bounds_resln = '
                '(0.6*resln_guess, 1.4*resln_guess)``.'
            ),
        ),
        ...
        'minmax_coeff_bounds': parset.set_parameter_definition(
            ...
            descr=(
                ...
                'sensfunc in the :func:`~pypeit.core.telluric.init_sensfunc_model` function. That '
                'determines a set of coefficients. The bounds are then determined according to: '
        ...
        'sticky': parset.set_parameter_definition(
            ...
            default=True,
            descr=(
                'Sticky parameter for the :func:`~pypeit.utils.djs_reject` algorithm for '
                'iterative model fit rejection.  If set to True then points rejected from a '
                'previous iteration are kept rejected, in other words the bad pixel mask is the '
                'OR of all previous iterations and rejected pixels accumulate.  If set to False, '
                'the bad pixel mask is the mask from the previous iteration, and if the model fit '
                'changes between iterations, points can alternate from being rejected to not '
                'rejected.  At present this code only performs optimizations with differential '
                'evolution and experience shows that sticky needs to be True in order for these '
                'to converge.  This is because the outliers can be so large that they dominate '
                'the loss function, and one never iteratively converges to a good model fit.  In '
                'other words, the deformations in the model between iterations with '
        ...
        'lower': parset.set_parameter_definition(
            ...
            default=3.0,
            descr=(
                r'Lower rejection threshold in units of :math:`\sigma_{\rm corr}\sigma`, where '
                r':math:`\sigma` is the formal noise of the spectrum, and :math:`\sigma_{\rm '
                'corr}` is an empirically determined correction to the formal error. The '
                r'distribution of input :math:`\chi` (defined by :math:`\chi = ({\rm data} - {\rm '
                r'model})/\sigma`) values is analyzed, and a correction factor to the formal '
        ...
        'upper': parset.set_parameter_definition(
            ...
            default=3.0,
            descr=(
                r'Upper rejection threshold in units of :math:`\sigma_{\rm corr}\sigma`, where '
                r':math:`\sigma` is the formal noise of the spectrum, and :math:`\sigma_{\rm '
                'corr}` is an empirically determined correction to the formal error. See '
...
class SensFuncPar(parset.ParSet):
    ...
    parameters = {
        ...
        'extr': parset.set_parameter_definition(
            ...
            default='OPT',
            descr=(
                'Extraction method to use for the sensitivity function.  Options are: "OPT" '
                '(optimal extraction), "BOX" (boxcar extraction). Default is "OPT".'
            ),
        ),
        ...
        'samp_fact': parset.set_parameter_definition(
            ...
            descr=(
                ...
                'coarser.  ``samp_fact`` > 1.0 oversamples (finer), ``samp_fact`` < 1.0 '
        ...
        'multi_spec_det': parset.set_parameter_definition(
            dtype=list,
            descr=(
                'List of detectors (identified by their string name, like "DET01") to splice '
                'together for multi-detector instruments (e.g. DEIMOS). It is assumed that there '
                'is *no* overlap in wavelength across detectors (might be ok if there is).  If '
                'entered as a list of integers, they should be converted to the detector name.  '
                '**Cannot be used with detector mosaics.**'
            ),
        ),
        'trim_std_pixs': parset.set_parameter_definition(
            dtype=[list, tuple],
            descr=(
                'List or tuple of two integers specifying the number of pixels to trim from the '
                'start and end of the 1D standard star spectrum. Example: [10, 5] will trim 10 '
                'pixels from the start (blue side) and 5 pixels from the end (red side) of the '
                'spectrum. '
            ),
        ),
        ...
        'star_arxiv': parset.set_parameter_definition(
            dtype=str,
            default='default',
            descr=(
                ...
                'prioritized list set by :func:`~pypeit.core.standard.get_archive_sets`.  Note '
                'this must be a valid, single string identifying the archive to use (e.g. '
                '``lbtmods``).'
            ),
        ),
    ...
    parameters = {
        'obj_toler': parset.set_parameter_definition(
            ...
            descr=(
                ...
                '(``use_maskdesign = True`` in ``EdgeTracePar``), this parameter provides the '
                'desired tolerance (arcsec) to match sources to targeted objects'
            ),
        ),
        ...
        'use_alignbox': parset.set_parameter_definition(
            ...
            descr=(
                ...
                '``True`` PypeIt will NOT compute the offset using ``snr_thrshd`` or '
                '``bright_maskdef_id``'
            ),
        ),
        ...
        'slitmask_offset': parset.set_parameter_definition(
            dtype=[int, float],
            descr=(
                'User-provided slitmask offset (pixels) from the position expected by the '
                'slitmask design. This is optional, and if set PypeIt will NOT compute the offset '
                'using ``snr_thrshd`` or ``bright_maskdef_id``.'
            ),
        ),
        'use_dither_offset': parset.set_parameter_definition(
            ...
            default=False,
            descr=(
                'Use the dither offset recorded in the header of science frames as the value of '
                'the slitmask offset. This is currently only available for Keck MOSFIRE reduction '
                'and it is set as the default for this instrument. If set PypeIt will NOT compute '
                'the offset using ``snr_thrshd`` or ``bright_maskdef_id``. However, it is ignored '
        ...
        'bright_maskdef_id': parset.set_parameter_definition(
            dtype=int,
            descr=(
                '`maskdef_id` (corresponding e.g., to `dSlitId` and `Slit_Number` in the '
                'DEIMOS/LRIS and MOSFIRE slitmask design, respectively) of a slit containing a '
                'bright object that will be used to compute the slitmask offset. This parameter '
                'is optional and is ignored if ``slitmask_offset`` is provided.'
            ),
        ),
        'extract_missing_objs': parset.set_parameter_definition(
            dtype=bool,
            default=False,
            descr='Force extraction of undetected objects at the location expected from the '
                  'slitmask design.',
        ),
        ...
        'missing_objs_boxcar_rad': parset.set_parameter_definition(
            dtype=[int, float],
            default=1.0,
            descr='Indicates the boxcar radius in arcsec for the force extraction of undetected '
                  'objects. ',
        ),
    }
...
class ReduxPar(parset.ParSet):
    ...
    parameters = {
        'spectrograph': parset.set_parameter_definition(
            dtype=str,
            descr=(
                'Spectrograph that provided the data to be reduced.  See :ref:`instruments` for '
                'valid options.'
            ),
        ),
        'quicklook': parset.set_parameter_definition(
            ...
            default=False,
            descr=(
                'Run a quick look reduction? This is usually good if you want to quickly reduce '
                'the data (usually at the telescope in real time) to get an initial estimate of '
                'the data quality.'
            ),
        ),
        'detnum': parset.set_parameter_definition(
            dtype=[int, list],
            descr=(
                'Restrict reduction to a list of detector indices. In case of mosaic reduction '
                '(currently only available for Gemini/GMOS and Keck/DEIMOS) ``detnum`` should be '
                'a list of tuples of the detector indices that are mosaiced together. E.g., for '
                'Gemini/GMOS ``detnum`` would be ``[(1,2,3)]`` and for Keck/DEIMOS it would be '
                '``[(1, 5), (2, 6), (3, 7), (4, 8)]``'
            ),
        ),
        ...
        'sortroot': parset.set_parameter_definition(
            dtype=str,
            descr=(
                'A filename given to output the details of the sorted files.  If None, the '
                'default is the root name of the pypeit file.  If off, no output is produced.'
            ),
        ),
        ...
        'chk_version': parset.set_parameter_definition(
            ...
            default=True,
            descr=(
                'If True enforce strict PypeIt version checking to ensure that all files were '
                'created with the current version of PypeIt.  If set to False, the code will '
                'attempt to read out-of-date files and keep going.  Beware (!!) that this can '
                'lead to unforeseen bugs that either cause the code to crash or lead to erroneous '
                'results. I.e., you really need to know what you are doing if you set this to '
                'False!'
            ),
        ),
    }
...
class WavelengthSolutionPar(parset.ParSet):
    ...
    parameters = {
        ...
        'method': parset.set_parameter_definition(
            ...
            options=valid_methods,
            descr=(
                'Method to use to fit the individual arc lines.  Note that some of the available '
                'methods should not be used; they are unstable and require significant parameter '
                'tweaking to succeed.  You should use one of "holy-grail", "reidentify", or '
                '"full_template".  "holy-grail" attempts to get a first guess at line IDs by '
                'looking for patterns in the line locations.  It is fully automated.  When it '
                'works, it works well; however, it can fail catastrophically.  Instead, '
                '"reidentify" and "full_template" are the preferred methods.  They require an '
                'archived wavelength solution for your specific instrument/grating combination as '
                'a reference.  This is used to anchor the wavelength solution for the data being '
        ...
        'ech_2dfit': parset.set_parameter_definition(
            ...
            descr=(
                ...
                'Set this to False if you wish to use the arxiv solution exactly as it was saved '
                'with pypeit_identify.'
            ),
        ),
        ...
        'ech_nspec_coeff': parset.set_parameter_definition(
            ...
            default=4,
            descr=(
                'For echelle spectrographs, this is the order of the final 2d fit to the spectral '
                'dimension.  You should choose this to be the ``n_final`` of the fits to the '
                'individual orders.'
            ),
        ),
        'ech_norder_coeff': parset.set_parameter_definition(
            ...
            default=4,
            descr=(
                'For echelle spectrographs, this is the order of the final 2d fit to the order '
                'dimension.'
            ),
        ),
        'ech_sigrej': parset.set_parameter_definition(
            ...
            default=2.0,
            descr=(
                'For echelle spectrographs, this is the sigma-clipping rejection threshold in the '
                '2d fit to spectral and order dimensions'
            ),
        ),
        ...
        'frac_rms_thresh': parset.set_parameter_definition(
            ...
            default=1.5,
            descr=(
                'For echelle spectrographs (i.e., ``echelle=True``), this is the fractional '
                'change in the RMS threshold used when a 1D fit is re-attempted for failed orders.'
            ),
        ),
        'lamps': parset.set_parameter_definition(
            dtype=list,
            descr=(
                'Name of one or more ions used for the wavelength calibration.  Use ``None`` for '
                'no calibration. Choose ``use_header`` to use the list of lamps recorded in the '
                'header of the arc frames (this is currently available only for Keck DEIMOS, Keck '
                'LRIS, MMT Blue Channel, and LDT DeVeny).'
            ),
        ),
        'lamps_wvrng': parset.set_parameter_definition(
            dtype=list,
            descr=(
                'Wavelength range of the lines to use from each lamp in ``lamps``, allowing a '
        ...
        'use_instr_flag': parset.set_parameter_definition(
            ...
            default=False,
            descr=(
                'If True, restrict to lines matching the instrument.  WARNING: This is only '
                'implemented for shane_kast_red + HolyGrail.  Do not use it unless you really '
        ...
        'sigdetect': parset.set_parameter_definition(
            ...
            default=5.0,
            descr=(
                'Sigma threshold above fluctuations for arc-line detection.  Arcs are continuum '
                'subtracted and the fluctuations are computed after continuum subtraction.  This '
                'can be a single number or a vector (list or numpy array) that provides the '
        ...
        'fwhm': parset.set_parameter_definition(
            ...
            default=4.0,
            descr=(
                'Spectral sampling of the arc lines. This is the FWHM of an arcline in binned '
                'pixels of the input arc image. Note that this is used also in the wave tilts '
                'calibration.'
            ),
        ),
        'fwhm_fromlines': parset.set_parameter_definition(
            ...
            default=True,
            descr=(
                'Estimate spectral resolution in each slit using the arc lines. If True, the '
                'estimated FWHM will override ``fwhm`` in the determination of the wavelength '
                'solution (including the calculation of the threshold for the solution RMS, see '
        ...
        'fwhm_spat_order': parset.set_parameter_definition(
            ...
            default=0,
            descr=(
                'This parameter determines the spatial polynomial order to use in the 2D '
                'polynomial fit to the FWHM of the arc lines. See also, ``fwhm_spec_order``.'
            ),
        ),
        'fwhm_spec_order': parset.set_parameter_definition(
            ...
            default=1,
            descr=(
                'This parameter determines the spectral polynomial order to use in the 2D '
                'polynomial fit to the FWHM of the arc lines. See also, ``fwhm_spat_order``.'
            ),
        ),
        'reid_arxiv': parset.set_parameter_definition(
            dtype=str,
            descr=(
                'Name of the archival wavelength solution file that will be used for the '
                'wavelength reidentification.  Only used if ``method`` is "reidentify" or '
                '"full_template".'
            ),
        ),
        ...
        'reid_cont_sub': parset.set_parameter_definition(
            ...
            default=True,
            descr=(
                'If True, continuum subtract the arc and arxiv spectrum before the wavelength '
                'reidentification. '
            ),
        ),
        ...
        'cc_shift_range': parset.set_parameter_definition(
            dtype=tuple,
            descr=(
                'Range of pixel shifts allowed when cross-correlating the input arc spectrum with '
                'the archive spectrum.  If None, ``cc_offset_minmax`` will be used to determine '
                'this range.'
            ),
        ),
        'cc_thresh': parset.set_parameter_definition(
            ...
            default=0.70,
            descr=(
                'Threshold for the *global* cross-correlation coefficient between an input '
                'spectrum and member of the archive required to attempt reidentification.  '
                'Spectra from the archive with a lower cross-correlation are not used for '
                'reidentification. This can be a single number or a list/array providing the '
                'value for each slit.'
            ),
        ),
        ...
        'match_toler': parset.set_parameter_definition(
            ...
            descr=(
                ...
                'shifted and stretched archive spectrum, and the archive wavelength solution at '
                'this match must be within ``match_toler`` dispersion elements from the line in '
                'line list.'
            ),
        ),
        'func': parset.set_parameter_definition(
        ...
        'n_final': parset.set_parameter_definition(
            ...
            default=4,
            descr=(
                'Order of final fit to the wavelength solution (there are ``n_final+1`` '
                'parameters in the fit). This can be a single number or a list/array providing '
                'the value for each slit'
            ),
        ),
        ...
        'reference_slit': parset.set_parameter_definition(
            ...
            descr=(
                ...
                'wavelength solution; (2) the greatest wavelength overlap with all slits; (3) the '
                'most lines correctly identified; and (4) the lowest RMS.  This parameter is the '
                'spatial ID of the slit to use as a reference for this process.'
            ),
        ),
        ...
        'echelle_pad': parset.set_parameter_definition(
            ...
            descr=(
                ...
                'method. Values > 0 allow for some error in the reddest order guess, but require '
                'sufficient reference orders.'
            ),
        ),
        'ech_angle_fits_file': parset.set_parameter_definition(
            dtype=str,
            descr=(
                'For the ``echelle`` method, the archive file with the fits of the per-order '
                'wavelength solutions vs the echelle and cross-disperser angles (e.g., '
                '``keck_hires_angle_fits.fits``). If None, the file is taken from the '
                'spectrograph class (``get_echelle_angle_files``).  Must be set together with '
                '``ech_composite_arc_file``.'
            ),
        ),
        'ech_composite_arc_file': parset.set_parameter_definition(
            dtype=str,
            descr=(
                'For the ``echelle`` method, the archive file with the composite arc spectrum of '
                'each order (e.g., ``keck_hires_composite_arc.fits``).  If None, the file is '
                'taken from the spectrograph class (``get_echelle_angle_files``).  Must be set '
                'together with ``ech_angle_fits_file``.'
            ),
        ),
        'ech_direct_cc': parset.set_parameter_definition(
            dtype=bool,
            default=False,
            descr=(
                'For the echelle method order identification, cross-correlate the stacked arc '
                'spectra directly instead of first building synthetic line-only arcs and '
                'continuum-subtracting the correlation function. Much faster for large spectral '
                'formats (e.g., 4k detectors with ~100 orders, such as Shane/Hamspec); the '
        ...
        'cc_offset_minmax': parset.set_parameter_definition(
            ...
            default=1.0,
            descr=(
                'Fraction of the total spectral pixels used to determine the range of pixel '
                'shifts allowed when cross-correlating the input arc spectrum with the archive '
                'spectrum. Restricting this can be crucial if there are few reference lines and '
                'the cross correlation can get confused. This parameter is only used if '
                '``cc_shift_range`` is None.'
            ),
        ),
        'stretch_func': parset.set_parameter_definition(
            ...
            descr=(
                'Whether to use a linear (linear) or quadratic (quad) function to stretch the '
                'extracted arcs when identifying emission lines with reidentify. For NIRSPEC, the '
                'quadratic mode tends to do better because the wavelength solution is typically '
                'at least 2nd or 3rd order.'
            ),
        ),
    }
...
class EdgeTracePar(parset.ParSet):
    ...
    parameters = {
        ...
        'sobel_mode': parset.set_parameter_definition(
            ...
            descr=(
                'Mode for Sobel filtering.  Default is "nearest"; note we find "constant" works '
                'best for DEIMOS.'
            ),
        ),
        ...
        'follow_span': parset.set_parameter_definition(
            ...
            default=20,
            descr=(
                'In the initial connection of spectrally adjacent edge detections, this sets the '
                'number of previous spectral rows to consider when following slits forward.'
            ),
        ),
        'det_min_spec_length': parset.set_parameter_definition(
            ...
            default=0.33,
            descr=(
                'The minimum spectral length (as a fraction of the detector size) of a trace '
                'determined by direct measurements of the detector data (as opposed to what '
                'should be included in any modeling approach; see ``fit_min_spec_length``).'
            ),
        ),
        'trim_spec': parset.set_parameter_definition(
            dtype=list,
            descr=(
                'User-defined truncation of all slits in the spectral direction. Should be two '
                'integers, e.g. 100,150 trims 100 pixels from the short wavelength end and 150 '
                'pixels from the long wavelength end of the spectral axis of the detector.'
            ),
        ),
        'mask_off_detector': parset.set_parameter_definition(
            ...
            default=False,
            descr=(
                'Mask spectral regions in each slit/order where more than 50% of the slit spatial '
                'coverage falls off the detector. '
            ),
        ),
        'max_shift_abs': parset.set_parameter_definition(
            ...
            default=0.5,
            descr=(
                'Maximum spatial shift in pixels between an input edge location and the '
                'recentroided value.'
            ),
        ),
        ...
        'match_tol': parset.set_parameter_definition(
            ...
            default=3.0,
            descr=(
                'Same-side slit edges below this separation in pixels are considered part of the '
                'same edge.'
            ),
        ),
        ...
        'fit_maxdev': parset.set_parameter_definition(
            ...
            default=5.0,
            descr=(
                'Maximum deviation between the fitted and measured edge position for rejection in '
                'spatial pixels.'
            ),
        ),
        ...
        'fit_min_spec_length': parset.set_parameter_definition(
            ...
            default=0.6,
            descr=(
                'Minimum unmasked spectral length of a traced slit edge to use in any modeling '
                'procedure (polynomial fitting or PCA decomposition).'
            ),
        ),
        ...
        'pca_min_edges': parset.set_parameter_definition(
            ...
            default=4,
            descr=(
                'Minimum number of edge traces required to perform a PCA decomposition of the '
                'trace form.  If ``left_right_pca`` is True, this minimum applies to the number '
                'of left and right traces separately.'
            ),
        ),
        'pca_n': parset.set_parameter_definition(
            dtype=int,
            descr=(
                'The number of PCA components to keep, which must be less than the number of '
                'detected traces.  If not provided, determined by calculating the minimum number '
                'of components required to explain a given percentage of variance in the edge '
                'data; see `pca_var_percent`.'
            ),
        ),
        'pca_var_percent': parset.set_parameter_definition(
            ...
            default=99.8,
            descr=(
                'The percentage (i.e., not the fraction) of the variance in the edge data '
                'accounted for by the PCA used to truncate the number of PCA coefficients to keep '
                '(see `pca_n`).  Ignored if `pca_n` is provided directly.'
            ),
        ),
        'pca_function': parset.set_parameter_definition(
            ...
            options=['polynomial', 'legendre', 'chebyshev'],
            descr=(
                'Type of function fit to the PCA coefficients for each component.  Options are: '
        ...
        'pca_sigrej': parset.set_parameter_definition(
            ...
            default=[2.0, 2.0],
            descr=(
                'Sigma rejection threshold for fitting PCA components. Individual numbers are '
                'used for both lower and upper rejection. A list of two numbers sets these '
                'explicitly (e.g., [2., 3.]).'
            ),
        ),
        ...
        'smash_range': parset.set_parameter_definition(
            ...
            default=[0.0, 1.0],
            descr=(
                'Range of the slit in the spectral direction (in fractional units) to smash when '
                'searching for slit edges.  If the spectrum covers only a portion of the image, '
                'use that range.'
            ),
        ),
        'edge_detect_clip': parset.set_parameter_definition(
            dtype=[int, float],
            descr=(
                'Sigma clipping level for peaks detected in the collapsed, Sobel-filtered '
                'significance image.'
            ),
        ),
        'trace_median_frac': parset.set_parameter_definition(
            dtype=[int, float],
            descr=(
                'After detection of peaks in the rectified Sobel-filtered image and before '
                'refitting the edge traces, the rectified image is median filtered with a kernel '
                'width of `trace_median_frac*nspec` along the spectral dimension.'
            ),
        ),
        'trace_thresh': parset.set_parameter_definition(
            dtype=[int, float],
            descr=(
                'After rectification and median filtering of the Sobel-filtered image (see '
                '`trace_median_frac`), values in the median-filtered image *below* this threshold '
                'are masked in the refitting of the edge trace data.  If None, no masking applied.'
            ),
        ),
        ...
        'fwhm_uniform': parset.set_parameter_definition(
            ...
            descr=(
                ...
                ':func:`~pypeit.core.trace.fit_trace` when refining the PCA predictions of '
                'edges.  See description of :func:`~pypeit.core.trace.peak_trace`.'
            ),
        ),
        'niter_uniform': parset.set_parameter_definition(
            ...
            default=9,
            descr=(
                'The number of iterations of :func:`~pypeit.core.trace.fit_trace` to use when '
        ...
        'det_buffer': parset.set_parameter_definition(
            ...
            default=5,
            descr=(
                'The minimum separation between the detector edges and a slit edge for any added '
                'edge traces.  Must be positive.'
            ),
        ),
        'max_nudge': parset.set_parameter_definition(
            dtype=[int, float],
            descr=(
                'If parts of any (predicted) trace fall off the detector edge, allow them to be '
                'nudged away from the detector edge up to and including this maximum number of '
                'pixels.  If None, no limit is set; otherwise should be 0 or larger.'
            ),
        ),
        'sync_predict': parset.set_parameter_definition(
            ...
            descr=(
                'Mode to use when predicting the form of the trace to insert.  Use `pca` to use '
                'the PCA decomposition, `nearest` to reproduce the shape of the nearest trace, or '
                '`auto` to let PypeIt decide which mode to use between `pca` and `nearest`. In '
                'general, it will first try `pca`, and if that is not possible, it will use '
                '`nearest`.'
            ),
        ),
        'sync_center': parset.set_parameter_definition(
            ...
            descr=(
                'Mode to use for determining the location of traces to insert.  Use `median` to '
                'use the median of the matched left and right edge pairs, `nearest` to use the '
                'length of the nearest slit, or `gap` to offset by a fixed gap width from the '
                'next slit edge.'
            ),
        ),
        'gap_offset': parset.set_parameter_definition(
            ...
            default=5.0,
            descr=(
                'Offset (pixels) used for the slit edge gap width when inserting slit edges (see '
                '`sync_center`) or when nudging predicted slit edges to avoid slit overlaps.  '
                'This should be larger than `minimum_slit_gap` when converted to arcseconds.'
            ),
        ),
        ...
        'bound_detector': parset.set_parameter_definition(
            ...
            default=False,
            descr=(
                'When the code is ready to synchronize the left/right trace edges, the traces '
                'should have been constructed, vetted, and cleaned. This can sometimes lead to '
                '*no* valid traces. This parameter dictates what to do next. If '
                '``bound_detector`` is True, the code will artificially add left and right edges '
                'that bound the detector; if False, the code identifies the slit-edge tracing as '
                'being unsuccessful, warns the user, and ends gracefully. Note that setting '
                '``bound_detector`` to True is needed for some long-slit data where the slit '
        ...
        'minimum_slit_dlength': parset.set_parameter_definition(
            dtype=[int, float],
            descr=(
                'Minimum *change* in the slit length (arcsec) as a function of wavelength in '
                'arcsec.  This is mostly meant to catch cases when the polynomial fit to the '
                'detected edges becomes ill-conditioned (e.g., when the slits run off the edge of '
                'the detector) and leads to wild traces.  If reducing the order of the polynomial '
                '(``fit_order``) does not help, try using this to remove poorly constrained slits.'
            ),
        ),
        'dlength_range': parset.set_parameter_definition(
            dtype=[int, float],
            descr=(
                'Similar to ``minimum_slit_dlength``, but constrains the *fractional* change in '
                'the slit length as a function of wavelength.  For example, a value of 0.2 means '
                'that slit length should not vary more than 20% as a function of wavelength.  '
            ),
        ),
        ...
        'minimum_slit_length_sci': parset.set_parameter_definition(
            dtype=[int, float],
            descr=(
                'Minimum slit length in arcsec for a science slit.  Slit lengths are determined '
                'by the median difference between the left and right edge locations for the '
                'unmasked trace locations.  Used in combination with ``minimum_slit_length``, '
                'this parameter is used to identify box or alignment slits; i.e., those slits '
                'that are shorter than ``minimum_slit_length_sci`` but larger than '
                '``minimum_slit_length`` are box/alignment slits.  Box slits are *never* removed '
                '(see ``clip``), but no spectra are extracted from them.  If None, no minimum '
                'science slit length is applied.'
            ),
        ),
        'length_range': parset.set_parameter_definition(
            dtype=[int, float],
            descr=(
                'Allowed range in slit length compared to the median slit length.  For example, a '
                'value of 0.3 means that slit lengths should not vary more than 30%.  Relatively '
                'shorter or longer slits are masked or clipped.  Most useful for echelle or '
                'multi-slit data where the slits should have similar or identical lengths.'
            ),
        ),
        'minimum_slit_gap': parset.set_parameter_definition(
            dtype=[int, float],
            descr=(
                'Minimum slit gap in arcsec.  Gaps between slits are determined by the median '
                'difference between the right and left edge locations of adjacent slits.  Slits '
                'with small gaps are merged by removing the intervening traces. If None, no '
                'minimum slit gap is applied.  This should be smaller than `gap_offset` when '
                'converted to pixels.'
            ),
        ),
        'clip': parset.set_parameter_definition(
            ...
            default=True,
            descr=(
                'Remove traces flagged as bad, instead of only masking them.  This is currently '
                'only used by :func:`~pypeit.edgetrace.EdgeTraceSet.centroid_refine`.'
            ),
        ),
        'order_match': parset.set_parameter_definition(
            dtype=[int, float],
            descr=(
                'Orders for *fixed-format* echelle spectrographs are always matched to a '
                'predefined expectation for the number of orders found and their relative '
                'placement in the detector.  This sets the tolerance allowed for matching '
                'identified "slits" to echelle orders. Must be relative to the fraction of the '
                'detector spatial scale (i.e., a value of 0.05 means that the order locations '
                'must be within 5% of the expected value).  If None, no limit is used.'
            ),
        ),
        'order_offset': parset.set_parameter_definition(
            ...
            descr=(
                'Orders for *fixed-format* echelle spectrographs are always matched to a '
                'predefined expectation for the number of orders found and their relative '
                'placement in the detector.  This sets the offset to introduce to the expected '
                'order positions to improve the match for this specific data. This is an additive '
                'offset to the measured slit positions; i.e., this should minimize the difference '
                'between the expected order positions and ``self.slit_spatial_center() + '
                'offset``. Must be in the fraction of the detector spatial scale. If None, no '
                'offset is applied.'
            ),
        ),
        ...
        'order_width_poly': parset.set_parameter_definition(
            ...
            default=2,
            descr=(
                'Order of the Legendre polynomial used to model the spatial width of each order '
                'as a function of spatial pixel position.  See ``add_missed_orders``.'
            ),
        ),
        'order_gap_poly': parset.set_parameter_definition(
            ...
            default=3,
            descr=(
                'Order of the Legendre polynomial used to model the spatial gap between orders as '
                'a function of the order spatial position.  See ``add_missed_orders``.'
            ),
        ),
        ...
        'order_outlier': parset.set_parameter_definition(
            dtype=[int, float],
            descr=(
                'When fitting the width of echelle orders with Legendre polynomials, this is the '
                'sigma-clipping threshold used to identify outliers.  Orders clipped by this '
        ...
        'order_spat_range': parset.set_parameter_definition(
            dtype=list,
            descr=(
                'The spatial range of the detector/mosaic over which to predict order locations.  '
                'If None, the full detector/mosaic range is used.  See ``add_missed_orders``.'
            ),
        ),
        'overlap': parset.set_parameter_definition(
            ...
            default=False,
            descr=(
                'Assume slits identified as abnormally short are actually due to overlaps between '
                'adjacent slits/orders.  If set to True, you *must* have also used '
                '``length_range`` to identify left-right edge pairs that have an abnormally short '
                'separation.  For those short slits, the code attempts to convert the short slits '
                'into slit gaps.  This is particularly useful for blue orders in Keck-HIRES data.'
            ),
        ),
        ...
        'maskdesign_maxsep': parset.set_parameter_definition(
            ...
            default=50,
            descr=(
                'Maximum allowed offset in pixels between the slit edges defined by the slit-mask '
                'design and the traced edges.'
            ),
        ),
        'maskdesign_step': parset.set_parameter_definition(
            ...
            descr=(
                ...
                '`maskdesign_maxsep`) between the slit edges defined by the mask design and the '
                'traced edges.'
            ),
        ),
        ...
        'maskdesign_trim': parset.set_parameter_definition(
            ...
            default=False,
            descr=(
                'If True, the mask design information is used to trim each slit in the spectral '
                'direction. This functionality is only used for spectrographs with slit-mask '
                'designs that have information on the spectral extent of each slit (currently, '
        ...
        'maskdesign_trim_shift': parset.set_parameter_definition(
            ...
            default=0,
            descr=(
                'Shift in pixels to apply to the mask design information when trimming the slits '
                'in the spectral direction.  This is useful for cases where the mask design '
                'information is not perfectly aligned with the detector.  This functionality is '
                'only used for spectrographs with slit-mask designs that have information on the '
                'spectral extent of each slit (currently, only Gemini GMOS N/S).'
            ),
        ),
        'pad': parset.set_parameter_definition(
            ...
            default=0.0,
            descr=(
                'Number of pixels to consider beyond the slit edges when generating a slitmask '
                'from the slit edges. Note that this parameter is *not* used to extend the slit '
                'edges themselves, but only to define the slitmask used for subsequent processing '
                ...
                '``10,20`` will extend the left edge by 10 pixels and the right edge by 20 '
                'pixels. If you provide a list with a single number, it will be used for both '
                'edges.'
            ),
        ),
        'add_slits': parset.set_parameter_definition(
            ...
            descr=(
                ...
                'detector 3 at spec=2000 extending from 1201 to 1500.  For mosaics, use the tuple '
                'definition of the mosaic.  For example, "(1,2,3):1537:297.2:353.5", adds a slit '
                'that passes through (1537,297.2) on the left and (1537,353.5) on the right in '
                'the mosaic made up of detectors 1, 2, and 3.'
            ),
        ),
        ...
        'rm_slits': parset.set_parameter_definition(
            dtype=[str, list],
            descr=(
                'Remove one or more user-specified slits.  The syntax used to define a slit to '
                'remove is: "det:spec:spat" where det=detector, spec=spectral pixel, spat=spatial '
...
class WaveTiltsPar(parset.ParSet):
    ...
    parameters = {
        'idsonly': parset.set_parameter_definition(
            ...
            default=False,
            descr=(
                'Only use the arc lines that have an identified wavelength to trace tilts '
                '(CURRENTLY NOT USED!)'
            ),
        ),
        'tracethresh': parset.set_parameter_definition(
            ...
            default=20.0,
            descr=(
                'Significance threshold for arcs to be used in tracing wavelength tilts. This can '
                'be a single number or a list/array providing the value for each slit/order.'
            ),
        ),
        ...
        'rm_continuum': parset.set_parameter_definition(
            ...
            default=False,
            descr=(
                'Before tracing the line center at each spatial position, remove any low-order '
                'continuum in the 2D spectra.'
            ),
        ),
    ...
    parameters = {
        ...
        'maxnumber_sci': parset.set_parameter_definition(
            ...
            descr=(
                ...
                'multislit ``maxnumber_sci`` applies per slit, for echelle observations this '
                'applies per order. Note that objects on a slit/order impact the sky-modeling and '
                'so ``maxnumber_sci`` should never be lower than the true number of detectable '
                'objects on your slit.  For image differenced observations with positive and '
                'negative object traces, ``maxnumber_sci`` applies to the number of positive (or '
                'negative) traces individually. In other words, if you had two positive objects '
        ...
        'ech_find_max_snr': parset.set_parameter_definition(
            ...
            descr=(
                ...
                'all the orders greater than this value or satisfy the min_snr criteria described '
                'by the min_snr parameters. If maxnumber is set (see above) then these criteria '
                'will be applied but only the maxnumber highest (median) S/N ratio objects will '
                'be kept.'
            ),
        ),
        ...
        'force_center_obj': parset.set_parameter_definition(
            dtype=bool,
            default=False,
            descr=(
                'If True, skip automated (peak-detection) object finding and instead force a '
                'single object at the center of each slit/order, with the FWHM and boxcar radius '
                'set by the slit width. Use this for spectrographs where the target always fills '
                'the slit (e.g., Shane/Hamspec), so the smashed spatial profile has no peak for '
                'the object finder to detect. Currently only implemented for Echelle reductions; '
                'it is ignored by the other pipelines.'
            ),
        ),
        ...
        'use_std_trace': parset.set_parameter_definition(
            ...
            default=True,
            descr=(
                'If True, the trace of the standard star spectrum is used as a crutch for tracing '
                'the object spectra. This is useful when a direct trace is not possible (i.e., '
                'faint sources). Note that a standard star exposure must be included in your '
        ...
        'std_spec1d': parset.set_parameter_definition(
            dtype=str,
            descr=(
                'A PypeIt spec1d file of a previously reduced standard star. This can be used to '
                'trace the object spectra, but the ``use_std_trace`` parameter must be set to '
                'True. If provided, this overrides use of any standards included in your pypeit '
                'file; the standard exposures will still be reduced.'
            ),
        ),
    }
...
class SkySubPar(parset.ParSet):
    ...
    parameters = {
        ...
        'user_regions': parset.set_parameter_definition(
            dtype=[str, list],
            descr=(
                'Provides a user-defined mask defining sky regions.  By default, the sky regions '
                'are identified automatically.  To specify sky regions for *all* slits, provide a '
                'comma separated list of percentages.  For example, setting ``user_regions = '
                ':10,35:65,80:`` selects the first 10%, the inner 30%, and the final 20% of *all* '
                'slits as containing sky.  Setting ``user_regions = user`` will attempt to load '
                'any SkyRegions files generated by the user via the ``pypeit_skysub_regions`` '
                'tool.'
            ),
        ),
        ...
        'joint_fit': parset.set_parameter_definition(
            ...
            descr=(
                ...
                'current implementation does not account for variations in the instrument FWHM in '
                'different slits. This will be addressed by Issue #1660.'
            ),
        ),
        ...
        'joint_fit_use_sci': parset.set_parameter_definition(
            dtype=bool,
            default=True,
            descr=(
                'Fiber pypeline only. When True, the global sky bspline is fit jointly to '
                'dedicated sky fibers and science fibers, using the standard iterative sigma '
                'rejection to down-weight pixels that contain real source flux. When False, only '
                'fibers whose MASKDEF_OBJNAME starts with "SKY" are used.'
            ),
        ),
        'sci_exclude_radius': parset.set_parameter_definition(
            dtype=[int, float],
            descr=(
                'Fiber pypeline only. When set and ``joint_fit_use_sci`` is True, exclude from '
                'the joint sky fit any science fiber whose on-sky position lies within this '
                'radius (in arcsec) of the instrument geometric center. Use this to exclude inner '
                'science fibers most likely to contain source flux while retaining fibers away '
                'from the source for additional sky background information.'
            ),
        ),
    }
...
class ExtractionPar(parset.ParSet):
    ...
    parameters = {
        ...
        'min_frac_prof': parset.set_parameter_definition(
            ...
            default=0.05,
            descr=(
                'For each spectral pixel, if the sum of the normalized object profile across the '
                'spatial direction is less than this value, the optimal extraction will also be '
                'masked. '
            ),
        ),
        'sn_gauss': parset.set_parameter_definition(
            ...
            descr=(
                ...
                'performs a b-spline fit to the object profile. For S/N < ``sn_gauss`` the code '
                'will simply optimal extract with a Gaussian with FWHM determined from the object '
                'finding.'
            ),
        ),
        ...
        'use_2dmodel_mask': parset.set_parameter_definition(
            ...
            default=True,
            descr=(
                'Mask pixels rejected during profile fitting when extracting. Turning this off '
                'may help with bright emission lines.'
            ),
        ),
        'use_user_fwhm': parset.set_parameter_definition(
            ...
            descr=(
                ...
                '(``find_fwhm`` in `FindObjPar`) for the optimal extraction. If this parameter is '
                '``False`` (default), PypeIt estimates the FWHM for each detected object, and '
                'uses ``find_fwhm`` as initial guess.'
            ),
        ),
...
class Collate1DPar(parset.ParSet):
    ...
    parameters = {
        'tolerance': parset.set_parameter_definition(
            ...
            default=1.0,
            descr=(
                'The tolerance used when comparing the coordinates of objects. If two objects are '
                'within this distance from each other, they are considered the same object. If '
                '``match_using`` is "ra/dec" (the default) this is an angular distance. The '
                'defaults units are arcseconds but other units supported by '
                'astropy.coordinates.Angle can be used (`e.g.`, "0.003d" or "0h1m30s"). If '
        ...
        'dry_run': parset.set_parameter_definition(
            ...
            default=False,
            descr=(
                'If set, the script will display the matching File and Object Ids but will not '
                'flux, coadd or archive.'
            ),
        ),
        ...
        'refframe': parset.set_parameter_definition(
            ...
            descr=(
                'Perform reference frame correction prior to coadding.  Options are: '
                f'{", ".join(valid_refframes)}'
            ),
        ),
    }
    ...
    parameters = {
        ...
        'flexure': parset.set_parameter_definition(
            ...
            descr=(
                ...
                '``flexure`` parameter group or set ``flexure = True`` in the ``rdx`` parameter '
                'group to use the default flexure-correction parameters.'
            ),
        ),
        'fluxcalib': parset.set_parameter_definition(
            ...
            descr=(
                'Parameters used by the flux-calibration procedure.  Flux calibration is not '
                'performed by default.  To turn on, either set the parameters in the '
                '``fluxcalib`` parameter group or set ``fluxcalib = True`` in the ``rdx`` '
        ...
        'telluric': parset.set_parameter_definition(
            ...
            default=TelluricPar(),
            descr=(
                'Par set to control telluric fitting.  Only used in the pypeit_sensfunc and '
                'pypeit_telluric after-burner scripts.'
            ),
        ),
