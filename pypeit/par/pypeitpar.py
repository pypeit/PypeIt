...
from pypeit.par.parset import ParSet, NewParSet
...
class NewProcessImagesPar(NewParSet):
    """
    New-style parameter set for basic image processing using `NewParSet`.

    This replaces the old instance-driven __init__ with a class-level
    `parameters` specification. The `NewParSet` base class handles defaulting,
    ...
    def validate(self):
        # Perform the same logical checks as the old implementation
        if self.data['n_lohi'] is not None and len(self.data['n_lohi']) != 2:
            raise ValueError('n_lohi must be a list of two numbers.')
        ...
        if not self.data['use_pixelflat'] and (self.data['use_illumflat'] or self.data['use_specillum']):
            raise ValueError('To apply a slit-illumination or spectral flat-field correction, you must also apply the pixel-flat correction.')



class NewScatteredLightPar(NewParSet):
    """
    The parameter set used to hold arguments for modelling the scattered light.
