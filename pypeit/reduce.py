...
class Reduce(object):
    ...
    def extract(self, global_sky, sobjs_obj):
        ...
        if self.par['reduce']['extraction']['skip_optimal']:  # Boxcar only with global sky subtraction
            ...
            self.outmask = self.sciImg.mask.fullmask
    ...
    def find_objects(self, image, std_trace=None,
        ...
        if show:
            self.show('image', image=image*(self.sciImg.mask.fullmask == 0), chname='objfind',sobjs=sobjs_obj_single, slits=True)
    ...
    def show(self, attr, image=None, showmask=False, sobjs=None,
        ...
        if showmask:
            mask_in = self.sciImg.mask.fullmask
            bitmask_in = self.sciImg.mask.bitmask
        else:
            mask_in = None
            bitmask_in = None
        ...
        if attr == 'global':
            ...
            if self.sciImg.image is not None and self.global_sky is not None and self.sciImg.mask.fullmask is not None:
                # sky subtracted image
                image = (self.sciImg.image - self.global_sky)*(self.sciImg.mask.fullmask == 0)
            ...
            if self.sciImg.image is not None and self.skymodel is not None \
                    ...
                    and self.sciImg.mask.fullmask is not None:
                image = (self.sciImg.image - self.skymodel) * np.sqrt(self.ivarmodel)
                image *= (self.sciImg.mask.fullmask == 0)
...
class MultiSlitReduce(Reduce):
    ...
    def find_objects_pypeline(self, image, std_trace=None,
        ...
        self.steps.append(inspect.stack()[0][3])
        if show:
            self.show('image', image=image*(self.sciImg.mask.fullmask == 0), chname = 'objfind',
...
class EchelleReduce(Reduce):
    ...
    def find_objects_pypeline(self, image, std_trace=None,
        ...
        inmask = self.sciImg.mask.fullmask == 0
        # Find objects
        ...
        self.steps.append(inspect.stack()[0][3])
        if show:
            self.show('image', image=image*(self.sciImg.mask.fullmask == 0), chname='ech_objfind',sobjs=sobjs_ech, slits=False)
    ...
    def local_skysub_extract(self, waveimg, global_sky, sobjs,
        ...
        self.skymodel, self.objmodel, self.ivarmodel, self.outmask, self.sobjs \
                = skysub.ech_local_skysub_extract(self.sciImg.image, self.sciImg.ivar,
                                                  self.sciImg.mask.fullmask, self.tilts, self.waveimg,
