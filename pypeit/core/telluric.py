...
def pca_lnprior(theta,pca_dict):
    """
    Routine to evaluate the the ln of the prior probability lnPrior, from the Gaussian mixture model fit
    to the distriution of PCA coefficients.
    Args:
        theta (`numpy.ndarray`_):
          Parameter vector, where theta_pca[0] is redshift, heta_pca[1] is the normalization, and
          theta_pca[2:npca+1] are the PCA coefficients, where npca is the PCA dimensionality
        pca_dict (dict):
          Dictionary continaing the PCA information generated by init_pca
    ...
    Returns:
        lnPrior (float):
        The log of the prior probability evaluated at the current set of coefficientsthe in the parameter vector, i.e.
        A = theta_pca[2:npca+1]
...
def eval_telluric(theta_tell, tell_dict, ind_lower=None, ind_upper=None):
    """
    Routine to evaluate the telluric model at an arbitrary location in the theta_tell parameter space.  The full
    atmosphere model lives in either a 5 or 6 dimensional parameter space, which is the size of the theta_tell input
    parameter vector.
    ...
    This routine performs the following steps:
       1) nearest grid point interpolation of the telluric model onto a new location in the 4-d space
       (pressure, temperature, humidity, airmass)
       2) convolution of the atmosphere model to the resolution set by resln.
       3) Optionally, if len(theta_tell) == 6, application of a shift to telluric model. If len(theta_tell) == 5,
          no shift is applied and only the first two steps are performed.

    Args:
        theta_tell (`numpy.ndarray`_):
            Five or six dimensional parameter vector describing the atmosphere. See above for description of parameters.
        tell_dict (dict):
            Dictionary containing the telluric grid.
        ind_lower (int):
            Lower index into the telluric model wave_grid to trim down the telluric model. This trimming makes things
            faster because we only need to convolve the portion that is needed for the current model fit.
        ind_upper:
            Upper index into the telluric model wave_grid to trim down the telluric model.

    Returns:
        telluric_model (`numpy.ndarray`_):
            Telluric model evaluated at the desired location theta_tell in atomphere parameter space
    """
