...
def eval_telluric(theta_tell, tell_dict, ind_lower=None, ind_upper=None):
    ...
    humidity, airmass, spectral resolution, shift, and stretch. The latter
    two can be omitted if these an shift and stretch of the telluric model
    are not included.
...
def sensfunc_telluric(wave, counts, counts_ivar, counts_mask, exptime, airmass, std_dict, telgridfile,
    ...
    for iord in range(norders):
        ...
        zeropoint_data_gpm =  TelObj.mask_grid[ind_lower:ind_upper + 1, iord]
        out_table[iord]['SENS_ZEROPOINT_GPM'][ind_lower:ind_upper+1] = zeropoint_data_gpm
        tellmodel_now = TelObj.tellmodel_list[iord]
        out_table[iord]['SENS_COUNTS_PER_ANG'][ind_lower:ind_upper + 1] = TelObj.flux_grid[ind_lower:ind_upper + 1, iord]
        flux_now = TelObj.flux_grid[ind_lower:ind_upper + 1, iord]
...
class Telluric(datamodel.DataContainer):
    ...
    Here f(x) represents the value of the loss function, which is essentially
    the chi^2 (but with the non-Gaussian tails remapped, and outliers
    rejected). Things are going well if the result of differential evolution
    yields a value of f(x) comparable to the number of pixels in your
    spectrum (or in the order in question for echelle data), which are the
    ...
    fits. Basically these telluric models are only good to about 3%, and so
    we cannot fit the data within the noise better than that, which is why
...
#        self.model = self.empty_model_table(self.norders, self.nspec, n_obj_par=self.n_obj_par)
        ...
        # 1) assignment of relevant input arguments
        ...
        # Turn on disp for the differential_evolution if debug mode is turned on.
        self.disp = disp | debug
        self.sensfunc = sensfunc
        self.debug = debug

        # 2) Reshape all spectra to be (nspec, norders)
        self.wave, self.flux, self.ivar, self.gpm, self.nspec, self.norders = \
                utils.spec_atleast_2d(wave, flux, ivar, gpm)

        # 3) Read the telluric grid and initalize associated parameters
        wv_gpm = self.wave > 1.0
        self.tell_dict = read_telluric_grid(self.telgridfile, wave_min=self.wave[wv_gpm].min(),
                                            wave_max=self.wave[wv_gpm].max())

        self.wave_grid = self.tell_dict['wave_grid']
#        self.ngrid = self.wave_grid.size
        self.resln_guess = wvutils.get_sampling(self.wave)[2] \
                                if resln_guess is None else resln_guess

        # Model parameter guess for determining the bounds with the
        # init_obj_model function
        ...
        self.flux_grid, self.ivar_grid, self.gpm_grid \
                = coadd.interp_spec(self.wave_grid, self.wave, self.flux, self.ivar, self.gpm,
                                    sensfunc=self.sensfunc)
        # This is a hack to get an interpolate mask indicating where
        # wavelengths are good on each order
        _ones = np.ones_like(self.flux)
        _, _, self.wave_grid_gpm = coadd.interp_spec(self.wave_grid, self.wave, _ones, _ones,
                                                     self.wave > 1.0)
        # Clip the ivar if that is requested (sn_clip = None simply returns the
        # ivar otherwise)
        self.ivar_grid = utils.clip_ivar(self.flux_grid, self.ivar_grid, self.sn_clip,
                                         gpm=self.gpm_grid)
        ...
        for counter, iord in enumerate(self.srt_order_tell):
            ...
            obj_dict, bounds_obj = init_obj_model(obj_params, iord,
                                ...
                                self.flux_grid[self.ind_lower[iord]:self.ind_upper[iord]+1,iord],
                                self.ivar_grid[self.ind_lower[iord]:self.ind_upper[iord]+1,iord],
                                self.mask_grid[self.ind_lower[iord]:self.ind_upper[iord]+1,iord],
                                tellmodel)
            ...
            ivar = self.ivar_grid[self.ind_lower[iord]:self.ind_upper[iord]+1,iord]
            arg_dict_iord = dict(ivar=ivar, tell_dict=self.tell_dict,
    ...
    def run(self, only_orders=None):
        ...
        for counter, iord in enumerate(self.srt_order_tell):
            ...
            self.result_list[iord], ymodel, ivartot, self.outmask_list[iord] = fitting.robust_optimize(
                self.flux_grid[self.ind_lower[iord]:self.ind_upper[iord]+1, iord], tellfit, self.arg_dict_list[iord],
                inmask=self.mask_grid[self.ind_lower[iord]:self.ind_upper[iord]+1, iord],
    ...
    def show_fit_qa(self, iord):
        ...
        flux_now = self.flux_grid[self.ind_lower[iord]:self.ind_upper[iord]+1, iord]
        sig_now = np.sqrt(utils.inverse(self.ivar_grid[self.ind_lower[iord]:self.ind_upper[iord]+1, iord]))
        mask_now = self.mask_grid[self.ind_lower[iord]:self.ind_upper[iord]+1, iord]
    ...
    def init_output(self):
        ...
        out_table['WAVE'] = np.zeros((self.norders, self.nspec))
        out_table['TELLURIC'] = np.zeros((self.norders, self.nspec))
        out_table['OBJ_MODEL'] = np.zeros((self.norders, self.nspec))
    ...
    def assign_output(self, iord):
        ...
        gdwave = self.wave[:,iord] > 1.0
        wave_in_gd = self.wave[gdwave,iord]
        wave_grid_now = self.wave_grid[self.ind_lower[iord]:self.ind_upper[iord]+1]
        self.out_table['WAVE'][iord] = self.wave[:,iord]
    ...
    def get_ind_lower_upper(self):
        ...
        wave_ma = np.ma.MaskedArray(np.tile(self.wave_grid, (self.norders,1)).T,
                                    mask=np.logical_not(self.wave_grid_gpm))
        # TODO: It would be useful if the upper index is actuall the stopping
...
#        return read_telluric_grid(self.telgridfile, wave_min=wave_min, wave_max=wave_max, pad_frac=pad_frac)
    ...
    def get_tell_guess(self):
        ...
        Returns:
            ...
            model grid.
        """
        return np.median(self.tell_dict['pressure_grid']), \
                np.median(self.tell_dict['temp_grid']), np.median(self.tell_dict['h2o_grid']), \
    ...
    def get_bounds_tell(self):
        ...
        return [(self.tell_dict['pressure_grid'].min(), self.tell_dict['pressure_grid'].max()),
                ...
                (self.resln_guess * self.resln_frac_bounds[0],
                 self.resln_guess * self.resln_frac_bounds[1]), self.pix_shift_bounds,
                self.pix_stretch_bounds]
    ...
    def sort_telluric(self):
        ...
        # Do a quick loop over all the orders to sort them in order of
        # strongest to weakest telluric absorption
        for iord in range(self.norders):
            tm_grid = self.tell_dict['tell_grid'][...,self.ind_lower[iord]:self.ind_upper[iord]+1]
            tell_med[iord] = np.mean(tm_grid[tm_grid.shape[0]//2, tm_grid.shape[1]//2,
                                             tm_grid.shape[2]//2, tm_grid.shape[3]//2])
        return np.argsort(tell_med)
