...
def fit_flat(flat, tilts_dict, tslits_dict_in, slit, inmask=None, spec_samp_fine=1.2,
    ...
    # slit_left_in
    # slit_righ_in
    # slit_img : with and without padding
    ...
    if good_frac < 0.5:
        ...
        return np.ones_like(flat), np.ones_like(flat), np.zeros_like(flat), tilts_dict['tilts'], \
                    thismask_in, slit_left_in, slit_righ_in
    ...
    # Create a tilts image using this padded thismask, rather than
    # using the original thismask_in slit pixels
    tilts = tracewave.fit2tilts(flat.shape, tilts_dict['coeffs'], tilts_dict['func2d'])
    ...
    if spec_frac < 0.5:
        msgs.warn('Spectral flatfield fit is to only {:4.2f}'.format(100*spec_frac)
    ...
    spec_set_fine, outmask_spec, specfit, _, exit_status \
            = utils.bspline_profile(pix_fit, log_flat_fit, log_ivar_fit, np.ones_like(pix_fit),
                                    inmask=inmask_log_fit, nord=4, upper=logrej, lower=logrej,
    ...
    if debug:
        ...
        plt.clf()
        ax = plt.gca()
        ax.plot(pix_fit,log_flat_fit, color='k', marker='o', markersize=0.4, mfc='k',
                fillstyle='full', linestyle='None', label='all pixels')
        ax.plot(pix_fit[was_fit_and_masked],log_flat_fit[was_fit_and_masked], color='red',
                marker='+', markersize=1.5, mfc='red', fillstyle='full', linestyle='None',
                label='masked')
        ax.plot(pix_fit, specfit, color='cornflowerblue', label = 'fit to blaze')
        ax.plot(spec_set_fine.breakpoints[goodbk], specfit_bkpt, color='lawngreen', marker='o',
                markersize=2.0, mfc='lawngreen', fillstyle='full', linestyle='None',
                label='bspline breakpoints')
    ...
    # Determine maximum counts in median filtered flat spectrum. Only
    # fit pixels > 0.1 of this maximum
    specfit_interp = interp1d(pix_fit, specfit, kind='linear', bounds_error=False,
                              fill_value=-np.inf)
    ...
    if spat_frac < 0.5:
        msgs.warn('Spatial flatfield fit is to only {:4.2f}'.format(100*spat_frac) 
                  + '% of the pixels on this slit.' + msgs.newline()
                  + '              Something apperas to be wrong here')
    ...
    # Use breakpoints at a spacing of a 1/10th of a pixel, but do not
    # allow a bsp smaller than the typical sampling
    ...
    norm_spec_spat[thismask] = flat[thismask]/np.fmax(spec_model[thismask], 1.0) \
                                    / np.fmax(illumflat[thismask],0.01)
    ...
    if debug:
        plt.clf()
        ax = plt.gca()
        ax.plot(ximg_fit, norm_spec_fit, color='k', marker='o', markersize=0.4, mfc='k',
                fillstyle='full',linestyle='None', label = 'all pixels')
        #ax.plot(ximg_fit[~imed], norm_spec_fit[~imed], color='darkred', marker='+',markersize=4.0,
        ...
        if tweak_slits:
            if tweak_dict['tweak_left']:
                ...
                plt.hlines(tweak_slits_thresh*tweak_dict['norm_max_left'], ximg_fit.min(), 0.5,
            ...
            if tweak_dict['tweak_righ']:
                ...
                plt.hlines(tweak_slits_thresh * tweak_dict['norm_max_righ'], 0.5, ximg_fit.max(),
