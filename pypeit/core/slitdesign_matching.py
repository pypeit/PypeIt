...
def best_offset(x_det, x_model, step=1, xlag_range=None):
    ...
    sdev = numpy.zeros((xlag.shape[0]))

    # -------- Loop over lags
    for j in range(xlag.shape[0]):
        ...
        nj = sind.shape[0]
        w1 = numpy.where(sind<x_det.shape[0])
        ...
        if x_det.shape[0] > 10:
            ...
            offs=numpy.zeros((x_det.shape[0]))
            for xelement in range(x_det.shape[0]):
            ...
            nbest2= nbest if nbest<x_det.shape[0] else x_det.shape[0]
...
def discrete_correlate_match(x_det, x_model, step=1, xlag_range=[-50, 50]):
    ...
    ind = numpy.zeros(x_det.shape[0], dtype=int)
    ...
    scale = 1 + coeff[1] if x_det.shape[0] > 4 else 1
    x_model_new *= scale
...
def slit_match(x_det, x_model, step=1, xlag_range=[-50,50], print_matches=False, edge=None):
    ...
    weights = numpy.zeros(residual.shape[0], dtype=int)
    weights[numpy.abs(residual) < 100.] = 1
    if weights.sum()==0: weights=numpy.ones(residual.shape[0], dtype=int)
    ...
    cut = 5 if res.shape[0]<5 else sigrej*sigres
    out = numpy.abs(res)> cut

    # check for duplicate indices
    bad = numpy.ones(ind.shape[0], dtype=int)
    bad[numpy.unique(ind, return_index=True)[1]] = 0
    wbad = numpy.where(bad)[0]
    if wbad.shape[0] > 0:
        print('Trimming {} bad matches in deimos_slit_match'.format(wbad.shape[0]))
        for i in range(wbad.shape[0]):
            badind = ind[wbad[i]]
            w = ind == badind
...
def plot_matches(edgetrace, ind, x_model, x_det, yref, slit_index, edge=None):
    yref_xdet=numpy.tile(yref, x_det.shape[0])
    yref_x_model=numpy.tile(yref, x_model.shape[0])
    ...
    for i in range(x_model[ind].shape[0]):
...
def slit_match_fix(need, ind):
    ...
    # synth = numpy.zeros(ind.shape[0], dtype=int)
    needadd = need.copy()
    needadd[ind] = False
    needind = numpy.where(needadd)[0]  # slits we are missing
    # if needind.shape[0] > 0:
