...
def sn_weights(flux, sig, mask, wave, dv_smooth=10000.0, const_weights=False, debug=False, verbose=False):
    ...
    sn_sigclip = stats.sigma_clip(sn_val_ma, sigma=3, iters=5)
...
def median_ratio_flux(spec, smask, ispec, iref, nsig=3., niter=5, **kwargs):
    ...
    # Clip
    mn_scale, med_scale, std_scale = stats.sigma_clipped_stats(med_flux, sigma=nsig, iters=niter, **kwargs)
    # Return
    return med_scale
...
def get_std_dev(irspec, rmask, ispec1d, s2n_min=2., wvmnx=None, **kwargs):
    ...
    std_dev = np.std(stats.sigma_clip(dev_sig, sigma=5, iters=2))
