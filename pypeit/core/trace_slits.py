...
def trace_refine(filt_image, edges, edges_mask, ncoeff=5, npca = None, pca_explained_var = 99.8, coeff_npoly_pca = 3,
    ...
    for key,sign in zip(['left','right'], [1., -1.]):
        ...
        trace_fweight, xfpos1, xferr1 = extract.iter_tracefit(np.fmax(sign*filt_image, -1.0*sign), trace_crutch, ncoeff, fwhm=3.0*fwhm, niter=9, return_extras=True)
        trace_gweight, xgpos1, xgerr1 = extract.iter_tracefit(np.fmax(sign*filt_image, -1.0*sign), trace_fweight, ncoeff, fwhm=fwhm,gweight=True, niter=6, return_extras=True)
