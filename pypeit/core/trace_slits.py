...
def trace_refine(filt_image, edges, edges_mask, ncoeff=5, npca = None, pca_explained_var = 99.8, coeff_npoly_pca = 3,
    ...
    for key,sign in zip(['left','right'], [1., -1.]):
        ...
        trace_fweight, _, _ = extract.iter_tracefit(np.fmax(sign*filt_image, 0.0), trace_crutch, ncoeff, fwhm=3.0*fwhm, niter=9)
        trace_gweight, _, _ = extract.iter_tracefit(np.fmax(sign*filt_image, 0.0), trace_fweight, ncoeff, fwhm=fwhm,gweight=True, niter=6)
