...
def get_manual_flexure(fitstbl, frame:int):
    ...
    file, which defaults to NaN; any finite value (including 0.) is
    interpreted as a user request to override the automatically
    computed spatial flexure.  See Issue #2180.
    ...
    Returns:
        :obj:`float` or None: The manual spatial flexure shift in
        pixels, or None if the user did not provide one.
    """
    # The 'shift' column may be absent entirely, e.g. for metadata
    # built without the user-added columns.
    if 'shift' not in fitstbl.keys():
        return None
    manual_flexure = fitstbl[frame]['shift']
    # NaN is the "not set" sentinel; see
    # PypeItMetaData.set_user_added_columns()
    return float(manual_flexure) if np.isfinite(manual_flexure) else None
...
def load_skyregions(spectrograph, fitstbl, par, frame, det, caliBrate,
                    calibrations_path:str, scifile:str=None,
