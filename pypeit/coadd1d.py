...
# NOTE: This was moved to a new module that only contains this class: pypeit/onespec.py
...
class CoAdd1D:
    ...
    def save(self, coaddfile, telluric=None, obj_model=None, overwrite=True):
        ...
        onespec = OneSpec(self.wave_coadd[wave_mask], self.flux_coadd[wave_mask],
                          PYP_SPEC=self.spectrograph.name, ivar=self.ivar_coadd[wave_mask],
...
class EchelleCoAdd1D(CoAdd1D):
    ...
    def coadd(self):
        ...
        """
        weights_sens = sensfunc.sensfunc_weights(self.sensfile, self.waves, debug=self.debug)
        (wave_coadd, flux_coadd, ivar_coadd, mask_coadd), order_stacks = coadd.ech_combspec(
            self.waves, self.fluxes, self.ivars, self.masks, weights_sens,
