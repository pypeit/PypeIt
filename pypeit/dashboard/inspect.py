...
# Processed-image steps viewed via pypeit_view_fits --proc (S3-Q13).
_PROCESSED_IMAGE_STEPS = ('bias', 'dark', 'arc', 'tiltimg')

# Steps whose own output_file is opened by a dedicated pypeit_chk_* tool.
# NOTE: wv_calib is NOT here — pypeit_chk_wavecalib only *prints* diagnostics
# to the terminal (no window), so the wv_calib output is viewed with ginga
# directly (per the Stage 3 Round-1 feedback).
_SINGLE_FILE_CHK = {
...
# Steps whose output is shown by opening the FITS in ginga directly.
_GINGA_OUTPUT_STEPS = ('wv_calib',)
...
def output_command(model, step, group, det):
    ...
    if step in _PROCESSED_IMAGE_STEPS:
        ...
        if spec is None or out is None:
            return None
        return ['pypeit_view_fits', spec, str(out), '--proc']
    ...
    if step in _GINGA_OUTPUT_STEPS:
        # e.g. wv_calib: open the FITS in ginga (pypeit_chk_wavecalib only
