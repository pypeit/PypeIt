...
(``pypeit_chk_*`` / ``pypeit_view_fits``) as subprocesses.  Those scripts
have **non-uniform** signatures, so this module exposes per-step *command
builders* rather than a flat map:

* ``bias`` / ``dark`` / ``arc`` / ``tiltimg`` — processed calibration images,
  opened **directly in ``ginga``** (these already-oriented PypeIt FITS do not
  display correctly through ``pypeit_view_fits``).
...
This module also builds the ``pypeit_run_to_calibstep`` argv used by the
Calibrations view's **(Re)Build** control, and the science-product viewer /
``pypeit_reduce_by_step`` / ``run_pypeit`` argv used by the Science view.
...
# Processed calibration image steps: opened directly in ginga
# (pypeit_view_fits does not display these processed outputs).
...
# image, so its "Inspect output" is disabled; its wavelength QA PNGs are
# shown in the QA list instead.
_SINGLE_FILE_CHK = {
...
def _spectrograph(model):
    ...
    """
    return model.header_info.spectrograph if model.header_info is not None \
        else None


def output_command(model, step, group, det):
    """
    Build the ``argv`` to inspect a step's processed **output**.
    ...
    -------
    :obj:`list` or None
        The command ``argv``, or ``None`` if it cannot be built (e.g. no
        output file / unknown spectrograph / step not viewable).
    """
    if step in _PROCESSED_IMAGE_STEPS:
        # The processed calibration FITS (Bias_*, Dark_*, Arc_*, Tiltimg_*)
        # are already-oriented PypeIt image files, so open them **directly
        # in ginga** — pypeit_view_fits (raw/inter) does not display these
        # processed outputs correctly.
...
def output_target(model, step, group, det):
    ...
    -------
    :obj:`pathlib.Path` or None
        The file that would be opened, or ``None`` if there is no viewer
        for this step.
    """
...
def view_input_command(model, input_file, det=None):
    """
    Build the ``argv`` to view a single **input** (raw) frame.

    Uses ``pypeit_view_fits --proc`` so the raw frame is oriented/trimmed and
    shown properly formatted (not the bare, unoriented FITS).
    ...
    model : :class:`~pypeit.dashboard.model.DashboardModel`
        The dashboard model.
    input_file : :obj:`str`
        Path to the raw input frame.
    det : :obj:`int`, :obj:`tuple`, :obj:`list`, optional
        The detector (int) or mosaic (tuple/list) the frame belongs to,
        passed to ``--det`` so the right detector is processed.

    Returns
    -------
    :obj:`list` or None
        The ``pypeit_view_fits`` command, or ``None`` if the spectrograph
        is unknown.
    """
...
def _det_run_arg(det):
    ...
    ----------
    det : :obj:`int`, :obj:`tuple`, :obj:`list`
        Detector (int) or mosaic (tuple/list of ints).
...
def run_command(model, step, group, det):
    ...
    ``pypeit_run_to_calibstep`` script.
    ...
    -------
    :obj:`list` or None
        The ``pypeit_run_to_calibstep`` command ``argv``, or ``None`` if
        the model has no ``.pypeit`` file.
    """
...
# -----------------------------------------------------------------------
# Science-product viewers + the science (Re)Build.
# -----------------------------------------------------------------------
...
def spec2d_command(spec2d_file, det=None):
    ...
    Parameters
    ----------
    spec2d_file : :obj:`str`, :obj:`pathlib.Path`
        Path to the ``spec2d`` product (or ``None``).
    det : :obj:`int`, :obj:`tuple`, :obj:`list`, optional
        Detector (int) or mosaic (tuple/list) to show.
...
def science_object_name(spat_pixpos, slitid, det_name):
    ...
    matching the format written by :meth:`pypeit.specobj.SpecObj.set_name`,
    so ``pypeit_show_1dspec`` can be pointed at a specific object.

    .. note::

        :meth:`pypeit.specobj.SpecObj.set_name` is the authoritative
        implementation of this format, but it is an instance method that
        requires a fully populated ``SpecObj``; the dashboard only has the
        (spatial position, slit, detector) values recorded in the reduction
        state, so the name is reconstructed here.  If a standalone
        name-construction utility is ever factored out of ``SpecObj``, use
        it here instead.

    Parameters
    ----------
    spat_pixpos : :obj:`float`
        The object's spatial pixel position.
    slitid : :obj:`int`
        The slit (SPAT_ID) the object is on.
    det_name : :obj:`str`
        The detector name (e.g. ``DET01``).
...
def spec1d_command(spec1d_file, obj_name=None):
    ...
    (``pypeit_show_1dspec <spec1d> [--obj <name>]``).  The user picks an
    object; pass its reconstructed name (see :func:`science_object_name`).

    Parameters
    ----------
    spec1d_file : :obj:`str`, :obj:`pathlib.Path`
        Path to the ``spec1d`` product (or ``None``).
    obj_name : :obj:`str`, optional
        The object extension name to plot.
...
def science_run_command(model, frame, det, step):
    ...
    model : :class:`~pypeit.dashboard.model.DashboardModel`
        The dashboard model.
    frame : :obj:`str`
        The science exposure basename (table key).
    det : :obj:`int`, :obj:`tuple`, :obj:`list`
        Detector (int) or mosaic (tuple/list).
    step : :obj:`str`
        One of ``process`` / ``findobj`` / ``extract``.

    Returns
    -------
    :obj:`list` or None
        The ``pypeit_reduce_by_step`` command ``argv``, or ``None`` if the
        model has no ``.pypeit`` file or no raw frame is known.
    """
    pypeit_file = getattr(model, 'pypeit_file', None)
    if pypeit_file is None:
        return None
...
def run_pypeit_command(model):
    ...
    process every science/standard frame, overwriting the existing outputs.
    This is the science analogue of a "build everything" action, distinct
    from the per-step (Re)Build controls.
    ...
    -------
    :obj:`list` or None
        The ``run_pypeit`` command ``argv`` (with ``-o`` to overwrite and
        ``--redux_path`` to write to the reduction directory), or ``None``
        if the model has no ``.pypeit`` file.
    """
    pypeit_file = getattr(model, 'pypeit_file', None)
    if pypeit_file is None:
        return None
