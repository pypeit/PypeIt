...
* ``bias`` / ``dark`` / ``arc`` / ``tiltimg`` — processed (intermediate)
  images, viewed with ``pypeit_view_fits <spec> <file> --inter`` (Stage 3b #1;
  ``--inter`` orients/formats the intermediate FITS, where bare ``ginga`` and
  ``--proc`` did not).
...
# Processed (intermediate) image steps: viewed with pypeit_view_fits --inter,
# which orients/formats the intermediate FITS (Stage 3b #1; bare ginga and
# --proc showed them unformatted / failed on these calibration files).
...
def output_command(model, step, group, det):
    ...
    if step in _PROCESSED_IMAGE_STEPS:
        # These are *intermediate* PypeIt image files (Arc_*, Tiltimg_*, …):
        # view them with pypeit_view_fits --inter, which formats/orients them
        # properly (bare ginga showed the raw FITS unformatted; Stage 3b #1).
        spec = _spectrograph(model)
        out = model.output_path(step, group, det)
        if spec is None or out is None:
            return None
        return ['pypeit_view_fits', spec, str(out), '--inter']
