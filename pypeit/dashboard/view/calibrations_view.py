...
class CalibrationsView(QWidget):
    ...
    def __init__(self, model, launcher=None, run_lock=None,
        ...
        # The step currently being (re)built, so its button shows the running
        # color (orange) until the run completes (Round-2 #2).
        self._rebuilding_step = None
    ...
    def _restyle_buttons(self):
        ...
        for step, button in self._step_buttons.items():
            ...
            # While a (re)build runs, show that step's button in the running
            # color (orange) with its hourglass glyph (Round-2 #2).
            if step == self._rebuilding_step:
    ...
    def _build_rebuild_button(self, step, group, det):
        ...
        # Distinct action color so it never reads as a status (C10/X1).
        action = palette.action_color(self._theme)
        button.setStyleSheet(
            f'background-color: {action}; color: {_text_on(action)}; '
            f'border-radius: 4px; padding: 4px 10px; font-weight: bold;')
        ...
        button.setEnabled(self._rebuild_available and not locked)
    ...
    def set_locked(self, locked):
        """
        Enable/disable the (Re)Build control when the run lock changes (X1).
        ...
        Returns:
            None.
        """
        if self._rebuild_button is not None:
            self._rebuild_button.setEnabled(
                self._rebuild_available and not locked)
    ...
    def _on_rebuild(self, step, group, det):
        ...
        if self._launcher is not None:
            # Mark the step running (orange) while the run is in progress.
            self._rebuilding_step = step
            self._restyle_buttons()
            self._launcher.run(
    ...
    def _after_rebuild(self, code, step, backups):
        ...
        # The run finished, so clear the running marker and keep the rebuilt
        # step selected through the refresh (Round-2 #2).
        self._rebuilding_step = None
        self._selected_step = step
