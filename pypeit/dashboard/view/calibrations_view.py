...
class CalibrationsView(QWidget):
    ...
    def _build_detail(self, step):
        ...
        # Inspect output (C8): enabled only when the output exists.
        self._add_output_control(step, group, det, entry)

        # Input files (C7): grouped for flats (S3-Q16), union otherwise.
    ...
    def _add_output_control(self, step, group, det, entry):
        ...
        available = argv is not None and entry is not None \
            and entry.status in ('success', 'complete')
        button = QPushButton('Inspect output')
        button.setEnabled(available)
        if available:
            button.clicked.connect(
                lambda _c=False, a=argv: self._launch(a, f'view {step}'))
    ...
    def _launch(self, argv, description):
        """
        ...
        if argv and self._launcher is not None:
            self._launcher.launch(argv, description=description)
