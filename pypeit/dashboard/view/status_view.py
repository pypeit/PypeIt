...
class StatusView(QWidget):
    ...
    def _build_summary(self):
        ...
        if len(sci) > 0:
            ...
            n_sci_fail = sum(any(row[col] == 'fail'
                                 for col in _SCI_STEP_COLS) for row in sci)
    ...
    def _build_science_placeholder(self):
        ...
        n_fail = sum(any(row[col] == 'fail' for col in _SCI_STEP_COLS)
                     for row in sci)
        summary = QLabel(
    ...
    def _build_science_navigator(self, sci):
        ...
        Args:
            sci (`astropy.table.Table`_): The science table
                (:meth:`science_table`).
        ...
        for i, row in enumerate(rows):
            statuses = {s: row[s] for s in _SCI_STRIP_STEPS}
            cell = ScienceNavCell(
    ...
    def _scoped_table(self, group, det):
        ...
        Returns:
            :obj:`list`: The scoped, step-ordered rows (one
            `astropy.table.Row`_ per step).
        """
        table = self._model.status_table()
    ...
    def _set_table_row(self, r, row):
        ...
        Args:
            ...
            row (`astropy.table.Row`_): A row of the scoped status table.
