...
the shared window-icon/font conventions, builds the main window from the
reduction metadata, and runs the Qt event loop.  The dashboard's loud-failure
exception hook is installed by the ``pypeit_dashboard`` script's
``init_log`` (see :meth:`pypeit.scripts.dashboard.PypeItDashboard.init_log`),
...
def launch(args):
    ...
    # derive it from the on-disk products (deriving may briefly block the UI
    # on launch, which is acceptable).  Edge states (missing/empty/malformed)
    # are reported via model.load_status and rendered by the Status view.
    ...
    # Keep the window within the available screen area: the default size
    # (1650x900) is wider than some displays, so clamp to ~95% of the
    ...
    timer = QTimer()
    timer.start(500)
    # A do-nothing timeout that returns control to the Python interpreter
    # every 500 ms, so a pending SIGINT (Ctrl+C) can be handled.
