...
class RunPypeItState(BaseModel):
    ...
    #def _broadcast_step(self, calib_id, det, step):
    #    """
    #    Emit a STEP-level log record describing the current calibration step.
#
#        Generated by JXP and Claude.
#
#        The payload is a pipe-delimited ``key=value`` string that the PypeIt
#        Dashboard parses to populate its status bar (Calibration ID, Detector,
#        Calibration Step, and the high-level Step).
#
#        Parameters
#        ----------
#        calib_id : :obj:`int`
#            The active calibration-group identifier.
#        det : :obj:`int` or :obj:`list`
#            The detector (or mosaic tuple) currently being processed.
#        step : :obj:`str`
#            The calibration step name (e.g. ``"bias"``, ``"arc"``).
#
#        Returns
#        -------
#        None
#        """
#        # Local import to avoid a circular import at module load time; ``log``
#        # is the shared PypeItLogger instance created in ``pypeit/__init__``.
#        from pypeit import log
#        log.step(f"calib_id={calib_id}|det={det}|calib_step={step}"
#                 f"|meta_step=Calibrations")
#
    def update_calib(self, step:str, calib_id: int, det: str, key:str, value,
        ...
        # Broadcast a STEP-level log record on each genuine transition so that
        # GUIs listening on the log stream (e.g. the PypeIt Dashboard) can
        # update their status display.  Done once per new step to avoid
        # spamming the log with the repeated metric/status updates that follow.
        #if step_changed and step in calib_classes:
        #    self._broadcast_step(calib_id, det, step)
        # Select items so far
