...
def test_calib_view_rebuild_running_marker_and_selection(tmp_path, qtbot,
                                                         monkeypatch):
    """
    A launched (re)build marks the step's button running (orange/hourglass);
    on completion the marker clears and the rebuilt step stays selected
    ...
    class _FakeLauncher:
        def run(self, argv, description=None, on_finished=None):
            pass                            # do not finish (stay "running")

    view._launcher = _FakeLauncher()
    monkeypatch.setattr(view, '_confirm_rebuild', lambda *a, **k: True)
    view._select_step('tilts')
    view._on_rebuild('tilts', 0, 1)
    assert view._rebuilding_step == 'tilts'
    assert '⏳' in view._step_buttons['tilts'].text()
    # Completion clears the marker and keeps the rebuilt step selected.
    view._after_rebuild(0, 'tilts', [])
    assert view._rebuilding_step is None
