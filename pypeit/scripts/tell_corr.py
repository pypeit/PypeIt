...
def parser(options=None):
    ...
    parser.add_argument("-r", "--redshift", type=float, default=0., help="Object redshift")
    parser.add_argument("-n", "--norder", type=float, default=0., help="Polynomial order")
...
def main(args):
    ...
    if args.algorithm is not None:
        par['telluric']['algorithm'] = args.algorithm
    if args.tell_grid is not None:
        par['telluric']['tell_grid'] = args.tell_grid
    ...
    if args.norder is not None:
        par['telluric']['norder'] = args.norder
    ...
    outfile = (os.path.basename(args.spec1dfile)).replace('.fits','tellcorr.fits')
    modelfile = (os.path.basename(args.spec1dfile)).replace('.fits','tellmodel.fits')

    # Run the telluric fitting procedure.
    if par['telluric']['algorithm']=='qso':
        # run telluric.qso_telluric to get the final results
        TelQSO = telluric.qso_telluric(args.spec1dfile, par['telluric']['tell_grid'], par['telluric']['pca_file'],
                                       par['telluric']['redshift'], modelfile, outfile,
                                       create_bal_mask=None, disp=args.plot, debug=args.debug, show=args.plot)
    elif par['telluric']['algorithm']=='poly':
        TelPoly = telluric.poly_telluric(args.spec1dfile, par['telluric']['tell_grid'], modelfile, outfile,
                                         polyorder=par['telluric']['norder'],
