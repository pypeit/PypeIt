...
class ShaneHamspecSpectrograph(spectrograph.Spectrograph):
    ...
    def compound_meta(self, headarr, meta_key):
        ...
        """
        if meta_key == 'mjd':
            return Time(_obs_isot(headarr[0]), format='isot').mjd
            ...
            # The header time is the start of the exposure; compute the
            # airmass at the exposure midpoint (see meta.airmass).
            obstime = Time(_obs_isot(headarr[0]), format='isot')
    ...
    def get_echelle_angle_files(self, meta_dict=None):
        ...
        needs its own archive; the era is chosen with the ``detector`` meta
        of the arc frame.
        ...
        Args:
            meta_dict (:obj:`dict`, optional):
                Metadata of the arc frame being calibrated.  If it contains
                a ``detector`` key, the era-matched archive files are
                returned; otherwise the (canonical) Loral-era files are.

        Returns:
            list: The angle_fits and composite_arc archive file names.
        """
        detector = 'Loral2Kx2K' if meta_dict is None \
            else meta_dict.get('detector', 'Loral2Kx2K')
...
def _obs_isot(hdr):
    ...
    data use ``DATE-OBS``.  Both record the START of the exposure.
