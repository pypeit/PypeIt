...
#                ax.set_xlim(-0.02, 1.02)
    ...
    def spatial_fit_finecorr(self, normed, onslit_tweak, slit_idx, slit_spat, gpm,
        ...
        if self.waveimg is None:
            log.warning(
                'Cannot perform the fine correction to the spatial illumination without the '
                'wavelength image.'
            )
    ...
    def spectral_illumination(self, gpm=None, debug=False):
        ...
        # check if the waveimg is available
        if self.waveimg is None:
            log.warn("Cannot perform the spectral illumination without the wavelength image.")
            return np.ones_like(self.rawflatimg.image)
