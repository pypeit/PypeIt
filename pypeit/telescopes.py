# encoding: utf-8
"""
Define the telescopes parameters used by PypeIt.

NOTE: Longitudes are measured increasing to the east, so west longitudes are negative.
"""
from pypeit.par import pypeitpar
from astropy.coordinates import EarthLocation
from astropy import units

#TODO: Remove 'Par' from class name?


class AATTelescopePar(pypeitpar.TelescopePar):
    def __init__(self):
        loc = EarthLocation.of_site('Siding Spring Observatory')
        super().__init__(
            name='AAT', longitude=loc.lon.to(units.deg).value,
            latitude=loc.lat.to(units.deg).value, elevation=loc.height.to(units.m).value,
            diameter=3.9, eff_aperture=12.0
        )


class GTCTelescopePar(pypeitpar.TelescopePar):
    def __init__(self):
        loc = EarthLocation.of_site('Roque de los Muchachos')
        super().__init__(
            name='GTC', longitude=loc.lon.to(units.deg).value,
            latitude=loc.lat.to(units.deg).value, elevation=loc.height.to(units.m).value,
            eff_aperture=73.0
        )


# eff_aperture of Keck, Shane from xidl
class KeckTelescopePar(pypeitpar.TelescopePar):
    def __init__(self):
        loc = EarthLocation.of_site('W. M. Keck Observatory')
        super().__init__(
            name='KECK', longitude=loc.lon.to(units.deg).value,
            latitude=loc.lat.to(units.deg).value, elevation=loc.height.to(units.m).value,
            fratio=15, diameter=10, eff_aperture=72.3674
        )


class SubaruTelescopePar(pypeitpar.TelescopePar):
    def __init__(self):
        loc = EarthLocation.of_site('Subaru Telescope')
        super().__init__(
            name='SUBARU', longitude=loc.lon.to(units.deg).value,
            latitude=loc.lat.to(units.deg).value, elevation=loc.height.to(units.m).value,
            fratio=12.2,  # CASSEGRAIN
            diameter=8.2, eff_aperture=52.3782  # This number is somewhat uncertain..
        )


class MagellanTelescopePar(pypeitpar.TelescopePar):
    def __init__(self):
        loc = EarthLocation.of_site('Las Campanas Observatory')
        super().__init__(
            name='MAGELLAN', longitude=loc.lon.to(units.deg).value,
            latitude=loc.lat.to(units.deg).value, elevation=loc.height.to(units.m).value,
            diameter=6.5
        )


class ShaneTelescopePar(pypeitpar.TelescopePar):
    def __init__(self):
        loc = EarthLocation.of_site('Lick Observatory')
        super().__init__(
            name='SHANE', longitude=loc.lon.to(units.deg).value,
            latitude=loc.lat.to(units.deg).value, elevation=loc.height.to(units.m).value,
            diameter=3.05, eff_aperture=6.3617
        )


class WHTTelescopePar(pypeitpar.TelescopePar):
    def __init__(self):
        loc = EarthLocation.of_site('Roque de los Muchachos')
        super().__init__(
            name='WHT', longitude=loc.lon.to(units.deg).value,
            latitude=loc.lat.to(units.deg).value, elevation=loc.height.to(units.m).value,
            diameter=4.2
        )

class INTTelescopePar(pypeitpar.TelescopePar):
    def __init__(self):
        loc = EarthLocation.of_site('Roque de los Muchachos')
        super().__init__(
            name='INT', longitude=loc.lon.to(units.deg).value,
            latitude=loc.lat.to(units.deg).value, elevation=loc.height.to(units.m).value,
            diameter=2.54
        )


class APFTelescopePar(pypeitpar.TelescopePar):
    def __init__(self):
        loc = EarthLocation.of_site('Lick Observatory')
        super().__init__(
            name='APF', longitude=loc.lon.to(units.deg).value,
            latitude=loc.lat.to(units.deg).value, elevation=loc.height.to(units.m).value,
            diameter=2.4
        )


class TNGTelescopePar(pypeitpar.TelescopePar):
    def __init__(self):
        loc = EarthLocation.of_site('Roque de los Muchachos')
        super().__init__(
            name='TNG', longitude=loc.lon.to(units.deg).value,
            latitude=loc.lat.to(units.deg).value, elevation=loc.height.to(units.m).value,
            diameter=3.58
        )


class VLTTelescopePar(pypeitpar.TelescopePar):
    def __init__(self):
        loc = EarthLocation.of_site('Paranal Observatory')
        super().__init__(
            name='VLT', longitude=loc.lon.to(units.deg).value,
            latitude=loc.lat.to(units.deg).value, elevation=loc.height.to(units.m).value,
            diameter=8.2, eff_aperture=51.2
        )
# VLT aperture from https://www.eso.org/observing/etc/doc/formulabook/node15.html
# This seems unrealistic given that pi(8.2^2)/4 = 52.81


class NTTTelescopePar(pypeitpar.TelescopePar):
    def __init__(self):
        loc = EarthLocation.of_site('La Silla Observatory')
        super().__init__(
            name='NTT', longitude=loc.lon.to(units.deg).value,
            latitude=loc.lat.to(units.deg).value, elevation=loc.height.to(units.m).value,
            diameter=3.58
        )


class GeminiNTelescopePar(pypeitpar.TelescopePar):
    def __init__(self):
        loc = EarthLocation.of_site('gemini_north')
        super().__init__(
            name='GEMINI-N', longitude=loc.lon.to(units.deg).value,
            latitude=loc.lat.to(units.deg).value, elevation=loc.height.to(units.m).value,
            diameter=8.1
        )


class GeminiSTelescopePar(pypeitpar.TelescopePar):
    def __init__(self):
        loc = EarthLocation.of_site('gemini_south')
        super().__init__(
            name='GEMINI-S', longitude=loc.lon.to(units.deg).value,
            latitude=loc.lat.to(units.deg).value, elevation=loc.height.to(units.m).value,
            diameter=8.1
        )

class SOARTelescopePar(pypeitpar.TelescopePar):
    def __init__(self):
        loc = EarthLocation.of_site('Cerro Pachon')
        super().__init__(
            name='SOAR', longitude=loc.lon.to(units.deg).value,
            latitude=loc.lat.to(units.deg).value, elevation=loc.height.to(units.m).value,
            diameter=4.1
        )                   # Ignores central obscuration


class LBTTelescopePar(pypeitpar.TelescopePar):
    def __init__(self):
        loc = EarthLocation.of_site('Large Binocular Telescope')
        super().__init__(
            name='LBT', longitude=loc.lon.to(units.deg).value,
            latitude=loc.lat.to(units.deg).value, elevation=loc.height.to(units.m).value,
            diameter=8.4
        )


class KPNOTelescopePar(pypeitpar.TelescopePar):
    def __init__(self):
        loc = EarthLocation.of_site('Kitt Peak National Observatory')
        super().__init__(
            name='KPNO', longitude=loc.lon.to(units.deg).value,
            latitude=loc.lat.to(units.deg).value, elevation=loc.height.to(units.m).value,
            diameter=4.0, eff_aperture=11.2
        )
# KPNO from https://en.wikipedia.org/wiki/Nicholas_U._Mayall_Telescope


class HiltnerTelescopePar(pypeitpar.TelescopePar):
    def __init__(self):
        loc = EarthLocation.of_site('mdm')
        super().__init__(
            name='HILTNER', longitude=loc.lon.to(units.deg).value,
            latitude=loc.lat.to(units.deg).value, elevation=loc.height.to(units.m).value,
            diameter=2.4
        )
# See https://noirlab.edu/public/programs/kitt-peak-national-observatory/the-hiltner-24m-telescope/


class MMTTelescopePar(pypeitpar.TelescopePar):
    def __init__(self):
        loc = EarthLocation.of_site('Whipple Observatory')
        super().__init__(
            name='MMT', longitude=loc.lon.to(units.deg).value,
            latitude=loc.lat.to(units.deg).value, elevation=loc.height.to(units.m).value,
            diameter=6.5
        )


class NOTTelescopePar(pypeitpar.TelescopePar):
    def __init__(self):
        loc = EarthLocation.of_site('Roque de los Muchachos')
        super().__init__(
            name='NOT', longitude=loc.lon.to(units.deg).value,
            latitude=loc.lat.to(units.deg).value, elevation=loc.height.to(units.m).value,
            diameter=2.56
        )


class P200TelescopePar(pypeitpar.TelescopePar):
    def __init__(self):
        loc = EarthLocation.of_site('Palomar')
        super().__init__(
            name='P200', longitude=loc.lon.to(units.deg).value,
            latitude=loc.lat.to(units.deg).value, elevation=loc.height.to(units.m).value,
            diameter=5.1
        )


# TODO: What's the diameter?
class BokTelescopePar(pypeitpar.TelescopePar):
    def __init__(self):
        loc = EarthLocation.of_site('Kitt Peak')
        super().__init__(
            name='BOK', longitude=loc.lon.to(units.deg).value,
            latitude=loc.lat.to(units.deg).value, elevation=loc.height.to(units.m).value
        )


class LDTTelescopePar(pypeitpar.TelescopePar):
    def __init__(self):
        loc = EarthLocation.of_site('Discovery Channel Telescope')
        super().__init__(
            name='LDT', longitude=loc.lon.to(units.deg).value,
            latitude=loc.lat.to(units.deg).value, elevation=loc.height.to(units.m).value,
            fratio=6.18, diameter=4.25, eff_aperture=12.37
        )


# TODO provisional values
class JWSTTelescopePar(pypeitpar.TelescopePar):
    def __init__(self):
        super().__init__(name='JWST', longitude=0.0, latitude=0.0, elevation=0.0, diameter=6.5)


class ARCTelescopePar(pypeitpar.TelescopePar):
    def __init__(self):
        loc = EarthLocation.of_site('Apache Point Observatory')
        super().__init__(
            name='APO', longitude=loc.lon.to(units.deg).value,
            latitude=loc.lat.to(units.deg).value, elevation=loc.height.to(units.m).value,
            diameter=3.5
        )
